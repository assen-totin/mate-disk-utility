...
G_DEFINE_TYPE (MduPort, mdu_port, G_TYPE_OBJECT);

MduPool *
mdu_port_get_pool (MduPort *port)
{
        return g_object_ref (port->priv->pool);
}

static void
mdu_port_finalize (MduPort *port)
{
...
static void
mdu_port_class_init (MduPortClass *klass)
{
        GObjectClass *obj_class = (GObjectClass *) klass;

        parent_class = g_type_class_peek_parent (klass);

        obj_class->finalize = (GObjectFinalizeFunc) mdu_port_finalize;

        g_type_class_add_private (klass, sizeof (MduPortPrivate));

        signals[CHANGED] =
                g_signal_new ("changed",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduPortClass, changed),
                              NULL, NULL,
        ...
        signals[REMOVED] =
                g_signal_new ("removed",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduPortClass, removed),
                              NULL, NULL,
                              g_cclosure_marshal_VOID__VOID,
                              G_TYPE_NONE, 0);
}
...
static void
mdu_port_init (MduPort *port)
{
        port->priv = G_TYPE_INSTANCE_GET_PRIVATE (port, MDU_TYPE_PORT, MduPortPrivate);
}

static gboolean
update_info (MduPort *port)
{
        PortProperties *new_properties;

        new_properties = port_properties_get (_mdu_pool_get_connection (port->priv->pool),
...
MduPort *
_mdu_port_new_from_object_path (MduPool *pool, const char *object_path)
{
        MduPort *port;

        port = MDU_PORT (g_object_new (MDU_TYPE_PORT, NULL));
        port->priv->object_path = g_strdup (object_path);
        port->priv->pool = g_object_ref (pool);

	port->priv->proxy = dbus_g_proxy_new_for_name (_mdu_pool_get_connection (port->priv->pool),
                                                       "org.freedesktop.UDisks",
                                                       port->priv->object_path,
        ...
        g_debug ("_mdu_port_new_from_object_path: %s", port->priv->props->native_path);
...
gboolean
_mdu_port_changed (MduPort *port)
{
        g_debug ("_mdu_port_changed: %s", port->priv->props->native_path);
        if (update_info (port)) {
...
const gchar *
mdu_port_get_object_path (MduPort *port)
{
...
const gchar *
mdu_port_get_native_path (MduPort *port)
{
        return port->priv->props->native_path;
}

const gchar *
mdu_port_get_adapter (MduPort *port)
{
        return port->priv->props->adapter;
}

const gchar *
mdu_port_get_parent (MduPort *port)
{
        return port->priv->props->parent;
}

gint
mdu_port_get_number (MduPort *port)
{
        return port->priv->props->number;
}

const gchar *
mdu_port_get_connector_type (MduPort *port)
{
