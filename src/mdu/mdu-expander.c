...
G_DEFINE_TYPE (MduExpander, mdu_expander, G_TYPE_OBJECT);

MduPool *
mdu_expander_get_pool (MduExpander *expander)
{
        return g_object_ref (expander->priv->pool);
}

static void
mdu_expander_finalize (MduExpander *expander)
{
...
static void
mdu_expander_class_init (MduExpanderClass *klass)
{
        GObjectClass *obj_class = (GObjectClass *) klass;

        parent_class = g_type_class_peek_parent (klass);

        obj_class->finalize = (GObjectFinalizeFunc) mdu_expander_finalize;

        g_type_class_add_private (klass, sizeof (MduExpanderPrivate));

        signals[CHANGED] =
                g_signal_new ("changed",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduExpanderClass, changed),
                              NULL, NULL,
        ...
        signals[REMOVED] =
                g_signal_new ("removed",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduExpanderClass, removed),
                              NULL, NULL,
                              g_cclosure_marshal_VOID__VOID,
                              G_TYPE_NONE, 0);
}
...
static void
mdu_expander_init (MduExpander *expander)
{
        expander->priv = G_TYPE_INSTANCE_GET_PRIVATE (expander, MDU_TYPE_EXPANDER, MduExpanderPrivate);
}

static gboolean
update_info (MduExpander *expander)
{
        ExpanderProperties *new_properties;

        new_properties = expander_properties_get (_mdu_pool_get_connection (expander->priv->pool),
...
MduExpander *
_mdu_expander_new_from_object_path (MduPool *pool, const char *object_path)
{
        MduExpander *expander;

        expander = MDU_EXPANDER (g_object_new (MDU_TYPE_EXPANDER, NULL));
        expander->priv->object_path = g_strdup (object_path);
        expander->priv->pool = g_object_ref (pool);

	expander->priv->proxy = dbus_g_proxy_new_for_name (_mdu_pool_get_connection (expander->priv->pool),
                                                           "org.freedesktop.UDisks",
                                                           expander->priv->object_path,
        ...
        g_debug ("_mdu_expander_new_from_object_path: %s", expander->priv->props->native_path);
...
gboolean
_mdu_expander_changed (MduExpander *expander)
{
        g_debug ("_mdu_expander_changed: %s", expander->priv->props->native_path);
        if (update_info (expander)) {
...
const gchar *
mdu_expander_get_object_path (MduExpander *expander)
{
...
const gchar *
mdu_expander_get_native_path (MduExpander *expander)
{
        return expander->priv->props->native_path;
}

const gchar *
mdu_expander_get_vendor (MduExpander *expander)
{
        return expander->priv->props->vendor;
}

const gchar *
mdu_expander_get_model (MduExpander *expander)
{
        return expander->priv->props->model;
}

const gchar *
mdu_expander_get_revision (MduExpander *expander)
{
        return expander->priv->props->revision;
}

guint
mdu_expander_get_num_ports (MduExpander *expander)
{
        return expander->priv->props->num_ports;
}

gchar **
mdu_expander_get_upstream_ports (MduExpander *expander)
{
        return expander->priv->props->upstream_ports;
}

const gchar *
mdu_expander_get_adapter (MduExpander *expander)
{
