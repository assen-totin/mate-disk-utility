...
typedef struct
{
        ...
        /* List of MduDevice objects currently being unmounted */
        GList *devices_being_unmounted;

        /* List of MduDevice objects with ATA SMART failures */
...
static void
on_device_removed (MduPool   *pool,
                   MduDevice *device,
                   gpointer   user_data)
{
        NotificationData *data = user_data;
...
static void
on_device_changed (MduPool   *pool,
                   MduDevice *device,
                   gpointer   user_data)
{
        NotificationData *data = user_data;
...
static void
on_device_job_changed (MduPool   *pool,
                       MduDevice *device,
                       gpointer   user_data)
{
        NotificationData *data = user_data;
...
notification_data_new (void)
{
        ...
        gtk_status_icon_set_from_icon_name (data->status_icon, "mdu-warning");
...
{
        MduDevice *device = MDU_DEVICE (user_data);
        GtkWidget *dialog;

        //g_debug ("show unmount dialog for %s", mdu_device_get_device_file (device));

        dialog = mdu_slow_unmount_dialog_new (NULL, device);
...
update_unmount_dialogs (NotificationData *data)
{
        ...
        devices = mdu_pool_get_devices (data->pool);
        ...
        for (l = devices; l != NULL; l = l->next) {
                ...
                if (!(mdu_device_job_in_progress (device) &&
                      g_strcmp0 (mdu_device_job_get_id (device), "FilesystemUnmount") == 0 &&
                      mdu_device_job_get_initiated_by_uid (device) == getuid ()))
                        continue;
...
/* TODO: keep in sync with src/mdu-gtk/mdu-ata-smart-dialog.c */
static gboolean
...
update_ata_smart_failures (NotificationData *data)
{
        ...
        for (l = devices; l != NULL; l = l->next) {
                ...
                available = mdu_device_drive_ata_smart_get_is_available (device);
                time_collected = mdu_device_drive_ata_smart_get_time_collected (device);
                status = mdu_device_drive_ata_smart_get_status (device);
        ...
        for (l = removed; l != NULL; l = l->next) {
                ...
                //g_debug ("%s is no longer failing", mdu_device_get_device_file (device));
        ...
        for (l = added; l != NULL; l = l->next) {
                ...
                //g_debug ("%s is now failing", mdu_device_get_device_file (device));
        }
...
show_menu_for_status_icon (NotificationData *data)
{
        ...
        for (l = data->ata_smart_failures; l != NULL; l = l->next) {
                ...
                presentable = mdu_pool_get_drive_by_device (data->pool, device);
                name = mdu_presentable_get_name (presentable);
                vpd_name = mdu_presentable_get_vpd_name (presentable);

                status = mdu_device_drive_ata_smart_get_status (device);
                status_desc = mdu_util_ata_smart_status_to_desc (status, &highlight, NULL, NULL);
                ...
                g_object_set_data_full (G_OBJECT (menu_item), "mdu-device", g_object_ref (device), g_object_unref);

                pixbuf = mdu_util_get_pixbuf_for_presentable (presentable, GTK_ICON_SIZE_SMALL_TOOLBAR);
