...
static gboolean add_pool (MduShell     *shell,
...
struct _MduShellPrivate
{
        ...
        MduPresentable *presentable_now_showing;
...
static MduSection *get_section_by_type (MduShell *shell, GType section_type);

G_DEFINE_TYPE (MduShell, mdu_shell, G_TYPE_OBJECT);

static void
mdu_shell_finalize (MduShell *shell)
{
...
static void
mdu_shell_class_init (MduShellClass *klass)
{
        GObjectClass *obj_class = (GObjectClass *) klass;

        parent_class = g_type_class_peek_parent (klass);

        obj_class->finalize = (GObjectFinalizeFunc) mdu_shell_finalize;

        g_type_class_add_private (klass, sizeof (MduShellPrivate));
}

static void create_window (MduShell        *shell);

static void
mdu_shell_init (MduShell *shell)
{
        shell->priv = G_TYPE_INSTANCE_GET_PRIVATE (shell, MDU_TYPE_SHELL, MduShellPrivate);

        shell->priv->pools = g_ptr_array_new ();
}

MduShell *
mdu_shell_new (const gchar *ssh_address)
{
        MduShell *shell;
        shell = MDU_SHELL (g_object_new (MDU_TYPE_SHELL, NULL));
...
GtkWidget *
mdu_shell_get_toplevel (MduShell *shell)
{
        return shell->priv->app_window;
}

MduPool *
mdu_shell_get_pool_for_selected_presentable (MduShell *shell)
{
        MduPool *pool;

        if (shell->priv->presentable_now_showing != NULL) {
                pool = mdu_presentable_get_pool (shell->priv->presentable_now_showing);
                g_object_unref (pool);
        } else {
                pool = NULL;
        }
...
MduPresentable *
mdu_shell_get_selected_presentable (MduShell *shell)
{
        return shell->priv->presentable_now_showing;
}

void
mdu_shell_select_presentable (MduShell *shell, MduPresentable *presentable)
{
        gboolean selected;
        MduPool *pool;
        ...
        if (MDU_IS_VOLUME (presentable)) {
                MduDevice *device;
                MduPresentable *p_to_select;
                ...
                if (device != NULL) {
                        if (mdu_device_is_partition (device)) {
                                ...
                                drive_device = mdu_pool_get_by_object_path (pool,
                                                                            mdu_device_partition_get_slave (device));
                                if (drive_device != NULL) {
                                        p_to_select = mdu_pool_get_drive_by_device (pool, drive_device);
                                        g_object_unref (drive_device);
                                }
                        } else {
                                p_to_select = mdu_pool_get_drive_by_device (pool, device);
                        }
                        g_object_unref (device);
                }

                if (p_to_select != NULL) {
                        MduSection *volumes_section;

                        mdu_pool_tree_view_select_presentable (MDU_POOL_TREE_VIEW (shell->priv->tree_view),
                                                               p_to_select);
                        gtk_widget_grab_focus (shell->priv->tree_view);

                        volumes_section = get_section_by_type (shell, MDU_TYPE_SECTION_VOLUMES);
                        g_warn_if_fail (volumes_section != NULL);
                        if (volumes_section != NULL) {
                                g_warn_if_fail (mdu_section_volumes_select_volume (MDU_SECTION_VOLUMES (volumes_section),
                                                                                   presentable));
                        }
                ...
                mdu_pool_tree_view_select_presentable (MDU_POOL_TREE_VIEW (shell->priv->tree_view), presentable);
                gtk_widget_grab_focus (shell->priv->tree_view);
                selected = TRUE;
        }
...
static GList *
compute_sections_to_show (MduShell *shell)
{
        GList *sections_to_show;

        sections_to_show = NULL;
        if (shell->priv->presentable_now_showing != NULL) {
                if (MDU_IS_HUB (shell->priv->presentable_now_showing)) {

                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_HUB);

                } else if (MDU_IS_LINUX_MD_DRIVE (shell->priv->presentable_now_showing)) {

                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_LINUX_MD_DRIVE);
                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_VOLUMES);

                } else if (MDU_IS_LINUX_LVM2_VOLUME_GROUP (shell->priv->presentable_now_showing)) {

                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_LINUX_LVM2_VOLUME_GROUP);
                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_VOLUMES);

                } else if (MDU_IS_DRIVE (shell->priv->presentable_now_showing)) {

                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_DRIVE);
                        sections_to_show = g_list_append (sections_to_show, (gpointer) MDU_TYPE_SECTION_VOLUMES);
                }
        }
...
update_title (MduShell *shell)
{
        ...
        device_file = NULL;
        if (device != NULL) {
                device_file = g_strdup (mdu_device_get_device_file (device));
                g_object_unref (device);
        }
 ...
 */
void
mdu_shell_update (MduShell *shell)
{
        static MduPresentable *last_presentable = NULL;
        ...
        if (shell->priv->presentable_now_showing == NULL) {
                ...
                reset_sections = TRUE;
        } else if (g_strcmp0 (mdu_presentable_get_id (shell->priv->presentable_now_showing),
                              mdu_presentable_get_id (last_presentable)) != 0) {
                reset_sections = TRUE;
        }
...
static MduSection *
get_section_by_type (MduShell *shell, GType section_type)
{
        GList *children;
        GList *l;
        MduSection *ret;
        ...
        for (l = children; l != NULL; l = l->next) {
                MduSection *section = MDU_SECTION (l->data);
...
static void
presentable_changed (MduPresentable *presentable, gpointer user_data)
{
        MduShell *shell = MDU_SHELL (user_data);
        mdu_shell_update (shell);
}

static void
presentable_job_changed (MduPresentable *presentable, gpointer user_data)
{
        MduShell *shell = MDU_SHELL (user_data);
        mdu_shell_update (shell);
}
...
device_tree_changed (GtkTreeSelection *selection, gpointer user_data)
{
        ...
        presentable = mdu_pool_tree_view_get_selected_presentable (MDU_POOL_TREE_VIEW (shell->priv->tree_view));
...
static void
presentable_added (MduPool *pool, MduPresentable *presentable, gpointer user_data)
{
        MduShell *shell = MDU_SHELL (user_data);
        mdu_shell_update (shell);
}
...
static void
presentable_removed (MduPool *pool, MduPresentable *presentable, gpointer user_data)
{
        MduShell *shell = MDU_SHELL (user_data);
        MduPresentable *enclosing_presentable;

        if (shell->priv->presentable_now_showing != NULL &&
            g_strcmp0 (mdu_presentable_get_id (presentable),
                       mdu_presentable_get_id (shell->priv->presentable_now_showing)) == 0) {
                ...
                if (enclosing_presentable != NULL) {
                        mdu_shell_select_presentable (shell, enclosing_presentable);
...
typedef struct {
        ...
        /* data obtained from MduCreateLinuxMdDialog */
        gchar *level;
        gchar *name;
        guint64 size;
        ...
        /* List of created components - MduDevice objects */
...
static void
md_raid_create_volume_cb (MduDrive     *drive,
                          GAsyncResult *res,
                          gpointer      user_data)
{
...
static void
new_linux_md_create_array_cb (MduPool    *pool,
                              ...
                              gpointer    user_data)
{
        ...
        if (error != NULL) {
                ...
                d = mdu_pool_get_by_object_path (pool, array_object_path);
                p = mdu_pool_get_drive_by_device (pool, d);
                mdu_shell_select_presentable (data->shell, p);
                g_object_unref (p);
                g_object_unref (d);
...
create_linux_md_do (CreateLinuxMdData *data)
{
        if (data->components->len == data->drives->len) {
                ...
                for (n = 0; n < data->components->len; n++) {
                        MduDevice *d = MDU_DEVICE (data->components->pdata[n]);
                        g_ptr_array_add (objpaths, (gpointer) mdu_device_get_object_path (d));
                }

                mdu_pool_op_linux_md_create (data->pool,
                                             objpaths,
                                             data->level,
                                             data->stripe_size,
                                             data->name,
                ...
                drive = MDU_DRIVE (data->drives->pdata[num_component]);
...
static GtkActionEntry entries[] = {
        ...
        {"file-create-linux-md-array", "mdu-raid-array", N_("_RAID Array..."), NULL, N_("Create a RAID array"), G_CALLBACK (new_linux_md_array_callback)},
...
static void
pool_disconnected (MduPool  *pool,
                   gpointer user_data)
{
        MduShell *shell = MDU_SHELL (user_data);
        ...
        mdu_pool_tree_model_set_pools (shell->priv->model, shell->priv->pools);
}

static gboolean
add_pool (MduShell     *shell,
          const gchar  *ssh_user_name,
          const gchar  *ssh_address,
          GError      **error)
{
        ...
        pool = mdu_pool_new_for_address (ssh_user_name, ssh_address, error);
        if (pool == NULL)
                goto out;
        ...
        if (shell->priv->model != NULL) {
                MduPresentable *selected_presentable;

                selected_presentable = mdu_shell_get_selected_presentable (shell);

                mdu_pool_tree_model_set_pools (shell->priv->model, shell->priv->pools);

                if (selected_presentable != NULL)
                        mdu_shell_select_presentable (shell, selected_presentable);
        }
