...
#include "mdu-edit-linux-lvm2-dialog.h"
#include "mdu-size-widget.h"

struct MduEditLinuxLvm2DialogPrivate
{
        gulong changed_id;
        gulong removed_id;
        ...
        MduDetailsElement *pv_smart_element;
        MduDetailsElement *pv_device_element;

        MduButtonElement *pv_new_button;
        MduButtonElement *pv_remove_button;
...
static void mdu_edit_linux_lvm2_dialog_constructed (GObject *object);

static void update_tree (MduEditLinuxLvm2Dialog *dialog);
static void update_details (MduEditLinuxLvm2Dialog *dialog);

static MduDevice *get_selected_pv (MduEditLinuxLvm2Dialog *dialog);
static gchar *get_selected_pv_uuid (MduEditLinuxLvm2Dialog *dialog);

G_DEFINE_TYPE (MduEditLinuxLvm2Dialog, mdu_edit_linux_lvm2_dialog, MDU_TYPE_DIALOG)

static void
mdu_edit_linux_lvm2_dialog_finalize (GObject *object)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (object);
        ...
        if (G_OBJECT_CLASS (mdu_edit_linux_lvm2_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_edit_linux_lvm2_dialog_parent_class)->finalize (object);
}

static void
mdu_edit_linux_lvm2_dialog_class_init (MduEditLinuxLvm2DialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduEditLinuxLvm2DialogPrivate));

        object_class->constructed  = mdu_edit_linux_lvm2_dialog_constructed;
        object_class->finalize     = mdu_edit_linux_lvm2_dialog_finalize;

        signals[NEW_BUTTON_CLICKED_SIGNAL] =
                g_signal_new ("new-button-clicked",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduEditLinuxLvm2DialogClass, new_button_clicked),
                              NULL,
                              NULL,
        ...
        signals[REMOVE_BUTTON_CLICKED_SIGNAL] =
                g_signal_new ("remove-button-clicked",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduEditLinuxLvm2DialogClass, remove_button_clicked),
                              NULL,
                              NULL,
...
static void
mdu_edit_linux_lvm2_dialog_init (MduEditLinuxLvm2Dialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog,
                                                    MDU_TYPE_EDIT_LINUX_LVM2_DIALOG,
                                                    MduEditLinuxLvm2DialogPrivate);
}

GtkWidget *
mdu_edit_linux_lvm2_dialog_new (GtkWindow                *parent,
                                MduLinuxLvm2VolumeGroup  *vg)
{
        g_return_val_if_fail (MDU_IS_LINUX_LVM2_VOLUME_GROUP (vg), NULL);
        return GTK_WIDGET (g_object_new (MDU_TYPE_EDIT_LINUX_LVM2_DIALOG,
...
static void
on_pv_new_button_clicked (MduButtonElement *button_element,
                          gpointer          user_data)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (user_data);
        g_signal_emit (dialog, signals[NEW_BUTTON_CLICKED_SIGNAL], 0);
}

static void
on_pv_remove_button_clicked (MduButtonElement *button_element,
                             gpointer          user_data)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (user_data);
        gchar *pv_uuid;
               ...
               gpointer         user_data)
{
        ...
        gchar *name;
        gchar *drive_name;
        MduPresentable *volume_for_pv;
        MduPresentable *drive_for_pv;
        gchar *pv_uuid;
        gchar *markup;
        gchar color[16];
        GtkStateType state;
        ...
        mdu_util_get_mix_color (GTK_WIDGET (dialog->priv->pvs_tree_view), state, color, sizeof (color));

        if (volume_for_pv != NULL) {
                name = mdu_presentable_get_vpd_name (volume_for_pv);
                if (drive_for_pv != NULL) {
                        drive_name = mdu_presentable_get_vpd_name (drive_for_pv);
                } else {
                        drive_name = g_strdup ("");
                }
        } else {
...
static void
on_linux_lvm2_vg_changed (MduPresentable   *presentable,
                          gpointer          user_data)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (user_data);
...
static void
on_linux_lvm2_vg_removed (MduPresentable   *presentable,
                          gpointer          user_data)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (user_data);
...
static void
mdu_edit_linux_lvm2_dialog_constructed (GObject *object)
{
        MduEditLinuxLvm2Dialog *dialog = MDU_EDIT_LINUX_LVM2_DIALOG (object);
        MduLinuxLvm2VolumeGroup *vg;
        ...
        icon = mdu_presentable_get_icon (MDU_PRESENTABLE (vg));
        ...
        name = mdu_presentable_get_name (MDU_PRESENTABLE (vg));
        vpd_name = mdu_presentable_get_vpd_name (MDU_PRESENTABLE (vg));
        ...
        if (G_OBJECT_CLASS (mdu_edit_linux_lvm2_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_edit_linux_lvm2_dialog_parent_class)->constructed (object);
}

/* ---------------------------------------------------------------------------------------------------- */

static MduDevice *
find_pv_by_uuid (MduPool     *pool,
                 const gchar *uuid)
{
        MduDevice *ret;
        GList *devices;
        ...
        for (l = devices; l != NULL; l = l->next) {
                ...
                if (mdu_device_should_ignore (d))
                        continue;

                if (mdu_device_is_linux_lvm2_pv (d) && g_strcmp0 (mdu_device_linux_lvm2_pv_get_uuid (d), uuid) == 0) {
                        ret = g_object_ref (d);
                        goto out;
                }
        }
...
static MduDevice *
get_selected_pv (MduEditLinuxLvm2Dialog *dialog)
{
        gchar *pv_uuid;
        MduPool *pool;
        MduDevice *pv;
...
static gchar *
get_selected_pv_uuid (MduEditLinuxLvm2Dialog *dialog)
{
        gchar *pv_uuid;
         GtkTreeSelection *tree_selection;
        GtkTreeIter iter;
...
static void
update_tree (MduEditLinuxLvm2Dialog *dialog)
{
        MduLinuxLvm2VolumeGroup *vg;
        GtkTreeStore *store;
        ...
        for (n = 0; pvs != NULL && pvs[n] != NULL; n++) {
                ...
                if (!mdu_linux_lvm2_volume_group_get_pv_info (vg,
                                                              pv_uuid,
                                                              NULL,
                                                              &size,
                                                              &allocated_size)) {
                ...
                if (pv != NULL) {
                        ...
                        volume_for_pv = mdu_pool_get_volume_by_device (pool, pv);

                        if (volume_for_pv == NULL) {
                                g_warning ("Cannot find volume for device `%s'", mdu_device_get_object_path (pv));
                                g_object_unref (pv);
                                continue;
                        }

                        if (mdu_device_is_partition (pv)) {
                                MduDevice *drive_device;
                                drive_device = mdu_pool_get_by_object_path (pool, mdu_device_partition_get_slave (pv));
                                if (drive_device != NULL) {
                                        drive_for_pv = mdu_pool_get_drive_by_device (pool, drive_device);
                                        g_object_unref (drive_device);
                                } else {
                                        g_warning ("No device with objpath %s", mdu_device_partition_get_slave (pv));
                                }
                        } else {
                                drive_for_pv = mdu_pool_get_drive_by_device (pool, pv);
                        }

                        pixbuf = mdu_util_get_pixbuf_for_presentable (volume_for_pv, GTK_ICON_SIZE_SMALL_TOOLBAR);

                        /* TODO: we can remove this once we export SIZE and ALLOCATED_SIZE */
                        if (size == G_MAXUINT64)
                                size = mdu_device_get_size (pv);
                ...
                if (size != G_MAXUINT64 && allocated_size != G_MAXUINT64)
                        unallocated_size_str = mdu_util_get_size_for_display (size - allocated_size, FALSE, FALSE);
                else
        ...
        for (l = slaves; l != NULL; l = l->next) {
                ...
                if (mdu_drive_is_active (MDU_DRIVE (linux_lvm2_drive))) {
                        position = mdu_device_linux_lvm2_pv_get_position (sd);
                        if (position >= 0) {
                ...
                slave_state_str = mdu_linux_lvm2_drive_get_slave_state_markup (linux_lvm2_drive, sd);
...
static void
update_details (MduEditLinuxLvm2Dialog *dialog)
{
        MduLinuxLvm2VolumeGroup *vg;
        MduDevice *pv;
        ...
        //MduLinuxLvm2DriveSlaveFlags slave_flags;
        ...
        if (pv == NULL) {
                mdu_details_element_set_text (dialog->priv->pv_smart_element, "–");
                mdu_details_element_set_icon (dialog->priv->pv_smart_element, NULL);
                mdu_details_element_set_text (dialog->priv->pv_device_element, "–");
                goto out;
        }
        ...
        if (mdu_device_is_partition (pv)) {
                MduPool *pool;
                pool = mdu_device_get_pool (pv);
                slave_drive_device = mdu_pool_get_by_object_path (pool, mdu_device_partition_get_slave (pv));
                g_object_unref (pool);
        } else {
        ...
        if (mdu_device_drive_ata_smart_get_is_available (slave_drive_device) &&
            mdu_device_drive_ata_smart_get_time_collected (slave_drive_device) > 0) {
                ...
                mdu_details_element_set_text (dialog->priv->pv_smart_element, s);
                mdu_details_element_set_icon (dialog->priv->pv_smart_element, icon);

                g_free (s);
                g_object_unref (icon);
        } else {
                /* Translators: Used when SMART is not supported on the RAID pv */
                mdu_details_element_set_text (dialog->priv->pv_smart_element, _("Not Supported"));
                icon = g_themed_icon_new ("mdu-smart-unknown");
                mdu_details_element_set_icon (dialog->priv->pv_smart_element, icon);
                g_object_unref (icon);
        }

        pv_device_file = mdu_device_get_device_file_presentation (pv);
        if (pv_device_file == NULL || strlen (pv_device_file) == 0)
                pv_device_file = mdu_device_get_device_file (pv);
        mdu_details_element_set_text (dialog->priv->pv_device_element, pv_device_file);

 out:
        mdu_button_element_set_visible (dialog->priv->pv_new_button, show_pv_new_button);
        mdu_button_element_set_visible (dialog->priv->pv_remove_button, show_pv_remove_button);
