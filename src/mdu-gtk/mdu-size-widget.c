...
static gdouble
mdu_unit_get_factor (MduUnit unit)
{
        gdouble ret;
...
static const gchar *
mdu_unit_get_name (MduUnit unit)
{
        const gchar *ret;
...
static MduUnit
mdu_unit_guess (const gchar *str)
{
        MduUnit ret;

        ret = MDU_UNIT_NOT_SET;
...
G_DEFINE_TYPE (MduSizeWidget, mdu_size_widget, GTK_TYPE_HBOX)

static void update_stepping (MduSizeWidget *widget);

static void
mdu_size_widget_finalize (GObject *object)
{
        //MduSizeWidget *widget = MDU_SIZE_WIDGET (object);

        if (G_OBJECT_CLASS (mdu_size_widget_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_size_widget_parent_class)->finalize (object);
}

static void
mdu_size_widget_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
mdu_size_widget_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
        ...
        switch (property_id) {
        case PROP_SIZE:
                mdu_size_widget_set_size (widget, g_value_get_uint64 (value));
                break;

        case PROP_MIN_SIZE:
                mdu_size_widget_set_min_size (widget, g_value_get_uint64 (value));
                break;

        case PROP_MAX_SIZE:
                mdu_size_widget_set_max_size (widget, g_value_get_uint64 (value));
                break;
                        ...
                        gpointer  user_data)
{
        ...
        ret = mdu_util_get_size_for_display ((guint64 ) value, FALSE, FALSE);
...
on_hscale_value_changed (GtkRange  *range,
                         gpointer   user_data)
{
        MduSizeWidget *widget = MDU_SIZE_WIDGET (user_data);
        guint64 old_size;
...
on_query_tooltip (GtkWidget  *w,
                  ...
                  gpointer    user_data)
{
        MduSizeWidget *widget = MDU_SIZE_WIDGET (w);
        gchar *s;
        gchar *s1;

        s1 = mdu_util_get_size_for_display (widget->priv->size, FALSE, TRUE);
                       ...
                       gpointer       user_data)
{
        ...
        unit_factor = mdu_unit_get_factor (widget->priv->unit);
        unit_name = mdu_unit_get_name (widget->priv->unit);
                      ...
                      gpointer       user_data)
{
        ...
        if (sscanf (entry_txt, "%lf", &val) == 1) {
                unit = mdu_unit_guess (entry_txt);
                if (unit != MDU_UNIT_NOT_SET) {
                        set_unit (widget, unit);
                }

                *out_new_val = val * mdu_unit_get_factor (widget->priv->unit);
                ret = TRUE;
        }
...
static void
mdu_size_widget_constructed (GObject *object)
{
        MduSizeWidget *widget = MDU_SIZE_WIDGET (object);
        ...
        if (G_OBJECT_CLASS (mdu_size_widget_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_size_widget_parent_class)->constructed (object);
}

static void
mdu_size_widget_init (MduSizeWidget *widget)
{
        GtkAdjustment *adjustment;

        widget->priv = G_TYPE_INSTANCE_GET_PRIVATE (widget,
                                                    MDU_TYPE_SIZE_WIDGET,
                                                    MduSizeWidgetPrivate);
        ...
        set_unit (widget, MDU_UNIT_GB);
}

static gboolean
mdu_size_widget_mnemonic_activate (GtkWidget *_widget,
                                   gboolean   group_cycling)
{
        MduSizeWidget *widget = MDU_SIZE_WIDGET (_widget);
        return gtk_widget_mnemonic_activate (widget->priv->hscale, group_cycling);
}

static void
mdu_size_widget_class_init (MduSizeWidgetClass *klass)
{
        ...
        g_type_class_add_private (klass, sizeof (MduSizeWidgetPrivate));

        gobject_class->get_property        = mdu_size_widget_get_property;
        gobject_class->set_property        = mdu_size_widget_set_property;
        gobject_class->constructed         = mdu_size_widget_constructed;
        gobject_class->finalize            = mdu_size_widget_finalize;

        widget_class->mnemonic_activate    = mdu_size_widget_mnemonic_activate;
        ...
        signals[CHANGED_SIGNAL] = g_signal_new ("changed",
                                                ...
                                                G_SIGNAL_RUN_LAST,
                                                G_STRUCT_OFFSET (MduSizeWidgetClass, changed),
                                                NULL,
                                                NULL,
...
GtkWidget *
mdu_size_widget_new (guint64 size,
                     guint64 min_size,
                     guint64 max_size)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_SIZE_WIDGET,
                                         "size", size,
                                         "min-size", min_size,
                                         "max-size", max_size,
                                         NULL));
...
static void
update_stepping (MduSizeWidget *widget)
{
        gdouble extent;
...
void
mdu_size_widget_set_size (MduSizeWidget *widget,
                          guint64        size)
{
        g_return_if_fail (MDU_IS_SIZE_WIDGET (widget));
...
void
mdu_size_widget_set_min_size (MduSizeWidget *widget,
                              guint64        min_size)
{
        g_return_if_fail (MDU_IS_SIZE_WIDGET (widget));
...
void
mdu_size_widget_set_max_size (MduSizeWidget *widget,
                              guint64        max_size)
{
        g_return_if_fail (MDU_IS_SIZE_WIDGET (widget));
...
guint64
mdu_size_widget_get_size     (MduSizeWidget *widget)
{
        g_return_val_if_fail (MDU_IS_SIZE_WIDGET (widget), 0);
        return widget->priv->size;
}

guint64
mdu_size_widget_get_min_size (MduSizeWidget *widget)
{
        g_return_val_if_fail (MDU_IS_SIZE_WIDGET (widget), 0);
...
guint64
mdu_size_widget_get_max_size (MduSizeWidget *widget)
{
        g_return_val_if_fail (MDU_IS_SIZE_WIDGET (widget), 0);
