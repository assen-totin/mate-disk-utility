...
static void mdu_format_dialog_constructed (GObject *object);

G_DEFINE_TYPE (MduFormatDialog, mdu_format_dialog, MDU_TYPE_DIALOG)

static void
mdu_format_dialog_finalize (GObject *object)
{
        MduFormatDialog *dialog = MDU_FORMAT_DIALOG (object);
        ...
        if (G_OBJECT_CLASS (mdu_format_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_format_dialog_parent_class)->finalize (object);
}

static void
mdu_format_dialog_get_property (GObject    *object,
                                guint       property_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
        ...
        case PROP_FS_LABEL:
                g_value_take_string (value, mdu_format_dialog_get_fs_label (dialog));
                break;

        case PROP_FS_OPTIONS:
                g_value_take_boxed (value, mdu_format_dialog_get_fs_options (dialog));
                break;

        case PROP_ENCRYPT:
                g_value_set_boolean (value, mdu_format_dialog_get_encrypt (dialog));
                break;

        case PROP_TAKE_OWNERSHIP:
                g_value_set_boolean (value, mdu_format_dialog_get_take_ownership (dialog));
                break;
...
static void
mdu_format_dialog_set_property (GObject      *object,
                                   guint         property_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
mdu_format_dialog_class_init (MduFormatDialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduFormatDialogPrivate));

        object_class->get_property = mdu_format_dialog_get_property;
        object_class->set_property = mdu_format_dialog_set_property;
        object_class->constructed  = mdu_format_dialog_constructed;
        object_class->finalize     = mdu_format_dialog_finalize;
...
static void
mdu_format_dialog_init (MduFormatDialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog,
                                                    MDU_TYPE_FORMAT_DIALOG,
                                                    MduFormatDialogPrivate);

        dialog->priv->table = gtk_table_new (2, 2, FALSE);
}

GtkWidget *
mdu_format_dialog_new (GtkWindow            *parent,
                       MduPresentable       *presentable,
                       MduFormatDialogFlags  flags)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_FORMAT_DIALOG,
...
GtkWidget *
mdu_format_dialog_new_for_drive (GtkWindow            *parent,
                                 MduDevice            *device,
                                 MduFormatDialogFlags  flags)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_FORMAT_DIALOG,
...
GtkWidget *
mdu_format_dialog_new_for_volume (GtkWindow            *parent,
                                  MduDevice            *device,
                                  MduFormatDialogFlags  flags)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_FORMAT_DIALOG,
...
gchar *
mdu_format_dialog_get_fs_type  (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), NULL);
        return g_strdup (dialog->priv->fs_type);
}

gchar *
mdu_format_dialog_get_fs_label (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), NULL);
...
gchar **
mdu_format_dialog_get_fs_options (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), NULL);
...
gboolean
mdu_format_dialog_get_encrypt  (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), FALSE);
        return dialog->priv->encrypt;
}

gboolean
mdu_format_dialog_get_take_ownership  (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), FALSE);
...
GtkWidget *
mdu_format_dialog_get_table (MduFormatDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_FORMAT_DIALOG (dialog), NULL);
...
update (MduFormatDialog *dialog)
{
        ...
        if (dialog->priv->flags & MDU_FORMAT_DIALOG_FLAGS_SIMPLE) {
                ...
                dialog->priv->fs_type = mdu_util_fstype_combo_box_get_selected (dialog->priv->fs_type_combo_box);
        ...
        max_label_len = 0;
        kfs = mdu_pool_get_known_filesystem_by_id (mdu_dialog_get_pool (MDU_DIALOG (dialog)), dialog->priv->fs_type);
        if (kfs != NULL) {
                max_label_len = mdu_known_filesystem_get_max_label_len (kfs);
                g_object_unref (kfs);

                if (! (dialog->priv->flags & MDU_FORMAT_DIALOG_FLAGS_SIMPLE)) {
                        if (mdu_known_filesystem_get_supports_unix_owners (kfs)) {
...
on_combo_box_changed (GtkWidget *combo_box,
                      gpointer   user_data)
{
        MduFormatDialog *dialog = MDU_FORMAT_DIALOG (user_data);
        update (dialog);
}
...
static void
mdu_format_dialog_constructed (GObject *object)
{
        MduFormatDialog *dialog = MDU_FORMAT_DIALOG (object);
        ...
        if (dialog->priv->flags & MDU_FORMAT_DIALOG_FLAGS_SIMPLE) {
                ...
                if (dialog->priv->flags & MDU_FORMAT_DIALOG_FLAGS_ALLOW_MSDOS_EXTENDED)
                        include_extended_partitions_for_scheme = "mbr";
                combo_box = mdu_util_fstype_combo_box_create (mdu_dialog_get_pool (MDU_DIALOG (dialog)),
                                                              include_extended_partitions_for_scheme);
                mdu_util_fstype_combo_box_select (combo_box, "ext4");
                dialog->priv->fs_type = g_strdup ("ext4");
                dialog->priv->fs_options = NULL;
                dialog->priv->encrypt = FALSE;
                dialog->priv->take_ownership = TRUE;
        ...
        if (! (dialog->priv->flags & MDU_FORMAT_DIALOG_FLAGS_SIMPLE)) {
                ...
                row++;
                mdu_util_fstype_combo_box_set_desc_label (dialog->priv->fs_type_combo_box,
	...
	gtk_box_pack_start (GTK_BOX (hbox), label, FALSE, FALSE, 0);
        ...
        if (G_OBJECT_CLASS (mdu_format_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_format_dialog_parent_class)->constructed (object);
}
