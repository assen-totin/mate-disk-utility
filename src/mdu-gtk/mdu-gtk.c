...
static ShowBusyData *
show_busy_data_new (MduDevice *device)
{
        ShowBusyData *data;
...
static int
process_compare (MduProcess *a, MduProcess *b)
{
        return mdu_process_get_id (a) - mdu_process_get_id (b);
}
...
show_busy_get_list_store (ShowBusyData *data, int *num_rows)
{
        ...
        store = gtk_list_store_new (3, GDK_TYPE_PIXBUF, G_TYPE_STRING, MDU_TYPE_PROCESS);
        error = NULL;
        processes = mdu_device_filesystem_list_open_files_sync (data->device, &error);
        if (error != NULL) {
        ...
        for (l = processes; l != NULL; l = l->next) {
                MduProcess *process = MDU_PROCESS (l->data);
                char *name;
                char *markup;
                GAppInfo *app_info;
                GdkPixbuf *pixbuf;
                ...
                app_info = mdu_process_get_app_info (process);
                pixbuf = NULL;
                if (app_info != NULL) {
                        GIcon *icon;
                        ...
                        command_line = mdu_process_get_command_line (process);
                ...
                if (mdu_process_get_owner (process) != getuid ()) {
                        s = g_strdup_printf (_("uid: %d  pid: %d  program: %s"),
                                             mdu_process_get_owner (process),
                                             mdu_process_get_id (process),
                                             mdu_process_get_command_line (process));
                } else {
...
gboolean
mdu_util_dialog_show_filesystem_busy (GtkWidget *parent_window,
                                      MduPresentable *presentable)
{
        ...
        if (mdu_device_is_partition (device)) {
                char *s;
                MduPresentable *enclosing_drive;
                enclosing_drive = mdu_presentable_get_enclosing_presentable (presentable);
                s = mdu_presentable_get_name (enclosing_drive);
                /* todo: icon list */
                window_icon = mdu_presentable_get_icon (enclosing_drive);
...
static void
mdu_util_dialog_secret_update (DialogSecretData *data)
{
...
static void
mdu_util_dialog_secret_entry_changed (GtkWidget *entry, gpointer user_data)
{
        DialogSecretData *data = user_data;
        mdu_util_dialog_secret_update (data);
}

static void
secret_dialog_device_removed (MduDevice *device, gpointer user_data)
{
...
static char *
mdu_util_dialog_secret_internal (GtkWidget   *parent_window,
        ...
        if (is_change_password || is_new_password) {
                ...
                if (data->password_entry != NULL)
                        g_signal_connect (data->password_entry, "changed",
                                          (GCallback) mdu_util_dialog_secret_entry_changed, data);
...
char *
mdu_util_dialog_ask_for_new_secret (GtkWidget      *parent_window,
                                    gboolean       *save_in_keyring,
                                    gboolean       *save_in_keyring_session)
{
        return mdu_util_dialog_secret_internal (parent_window,
                                                _("Enter Passphrase"),
                                                NULL,
                                                TRUE,
                                                FALSE,
...
/**
 * mdu_util_dialog_ask_for_secret:
 * @parent_window: Parent window that dialog will be transient for or #NULL.
 * @presentable: A #MduPresentable with a #MduDevice that is encrypted.
 ...
 **/
char *
mdu_util_dialog_ask_for_secret (GtkWidget      *parent_window,
                                MduPresentable *presentable,
                                ...
                                gboolean       *asked_user)
{
        ...
        secret = mdu_util_dialog_secret_internal (parent_window,
                                                  window_title,
                                                  window_icon,
                                                  FALSE,
                                                  FALSE,
 ...
 * mdu_util_delete_secret() and mdu_util_save_secret().
 *
 * Returns: #TRUE if the user agreed to change the secret.
 **/
gboolean
mdu_util_dialog_change_secret (GtkWidget       *parent_window,
                               MduPresentable  *presentable,
                               char           **old_secret,
                               char           **new_secret,
                               gboolean        *save_in_keyring,
                               ...
                               gboolean         indicate_wrong_passphrase)
{
        ...
        *new_secret = mdu_util_dialog_secret_internal (parent_window,
                                                       window_title,
                                                       window_icon,
                                                       FALSE,
                                                       TRUE,
...
/**
 * mdu_util_delete_confirmation_dialog:
 ...
 **/
gboolean
mdu_util_delete_confirmation_dialog (GtkWidget   *parent_window,
                                     const char  *title,
...
static void
mdu_util_fstype_combo_box_update_desc_label (GtkWidget *combo_box)
{
        GtkWidget *desc_label;

        desc_label = g_object_get_data (G_OBJECT (combo_box), "mdu-desc-label");
        if (desc_label != NULL) {
                char *s;
                char *fstype;
                char *desc;

                fstype = mdu_util_fstype_combo_box_get_selected (combo_box);
                if (fstype != NULL) {
                        desc = mdu_util_fstype_get_description (fstype);
                        if (desc != NULL) {
...
static GtkListStore *
mdu_util_fstype_combo_box_create_store (MduPool *pool, const char *include_extended_partitions_for_scheme)
{
        GList *l;
        GtkListStore *store;
        ...
        if (pool != NULL)
                known_filesystems = mdu_pool_get_known_filesystems (pool);
        else
                known_filesystems = NULL;
        for (l = known_filesystems; l != NULL; l = l->next) {
                MduKnownFilesystem *kfs = l->data;
                const char *fstype;
                char *fstype_name;

                fstype = mdu_known_filesystem_get_id (kfs);

                if (strcmp (fstype, "empty") == 0) {
                        fstype_name = g_strdup (_("Empty (don't create a file system)"));
                } else {
                        fstype_name = mdu_util_get_fstype_for_display (fstype, NULL, TRUE);
                        /* TODO */
                }
...
void
mdu_util_fstype_combo_box_set_desc_label (GtkWidget *combo_box, GtkWidget *desc_label)
{
        ...
        mdu_util_fstype_combo_box_update_desc_label (combo_box);
}

void
mdu_util_fstype_combo_box_rebuild (GtkWidget  *combo_box,
                                   MduPool *pool,
                                   const char *include_extended_partitions_for_scheme)
{
        GtkListStore *store;
        store = mdu_util_fstype_combo_box_create_store (pool, include_extended_partitions_for_scheme);
...
static void
mdu_util_fstype_combo_box_changed (GtkWidget *combo_box, gpointer user_data)
{
        mdu_util_fstype_combo_box_update_desc_label (combo_box);
}

/**
 * mdu_util_fstype_combo_box_create:
 * @pool: A #MduPool object
 ...
 **/
GtkWidget *
mdu_util_fstype_combo_box_create (MduPool *pool, const char *include_extended_partitions_for_scheme)
{
        GtkListStore *store;
	GtkCellRenderer *renderer;
        GtkWidget *combo_box;
	...
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (combo_box), renderer,
        ...
        g_signal_connect (combo_box, "changed", (GCallback) mdu_util_fstype_combo_box_changed, NULL);

        return combo_box;
}

gboolean
mdu_util_fstype_combo_box_select (GtkWidget *combo_box, const char *fstype)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
        gboolean ret;
...
char *
mdu_util_fstype_combo_box_get_selected (GtkWidget *combo_box)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
        char *fstype;
...
static GtkListStore *
mdu_util_part_type_combo_box_create_store (const char *part_scheme)
{
        GtkListStore *store;
        PartTypeData data;
        ...
        mdu_util_part_type_foreach (part_type_foreach_cb, &data);
...
void
mdu_util_part_type_combo_box_rebuild (GtkWidget  *combo_box, const char *part_scheme)
{
        GtkListStore *store;
        store = mdu_util_part_type_combo_box_create_store (part_scheme);
	gtk_combo_box_set_model (GTK_COMBO_BOX (combo_box), GTK_TREE_MODEL (store));
        g_object_unref (store);
        gtk_combo_box_set_active (GTK_COMBO_BOX (combo_box), 0);
}
...
/**
 * mdu_util_part_type_combo_box_create:
 ...
 **/
GtkWidget *
mdu_util_part_type_combo_box_create (const char *part_scheme)
{
        GtkListStore *store;
	GtkCellRenderer *renderer;
        GtkWidget *combo_box;
...
gboolean
mdu_util_part_type_combo_box_select (GtkWidget *combo_box, const char *part_type)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
        gboolean ret;
...
char *
mdu_util_part_type_combo_box_get_selected (GtkWidget *combo_box)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
        char *part_type;
...
static GtkListStore *
mdu_util_part_table_type_combo_box_create_store (void)
{
        GtkListStore *store;
        GtkTreeIter iter;
...
static void
mdu_util_part_table_type_combo_box_update_desc_label (GtkWidget *combo_box)
{
        GtkWidget *desc_label;
        ...
        if (desc_label != NULL) {
                ...
                fstype = mdu_util_part_table_type_combo_box_get_selected (combo_box);
                if (fstype != NULL) {
                        desc = mdu_util_part_table_type_get_description (fstype);
                        if (desc != NULL) {
                                s = g_strdup_printf ("<small><i>%s</i></small>", desc);
                                gtk_label_set_markup (GTK_LABEL (desc_label), s);
                                g_free (s);
...
static void
mdu_util_part_table_type_combo_box_changed (GtkWidget *combo_box, gpointer user_data)
{
        mdu_util_part_table_type_combo_box_update_desc_label (combo_box);
}

void
mdu_util_part_table_type_combo_box_set_desc_label (GtkWidget *combo_box, GtkWidget *desc_label)
{
        g_object_set_data_full (G_OBJECT (combo_box),
                                "mdu-desc-label",
                                g_object_ref (desc_label),
...
/**
 * mdu_util_part_table_type_combo_box_create:
 *
 ...
 **/
GtkWidget *
mdu_util_part_table_type_combo_box_create (void)
{
        GtkListStore *store;
	GtkCellRenderer *renderer;
        GtkWidget *combo_box;
        ...
        combo_box = gtk_combo_box_new ();
        store = mdu_util_part_table_type_combo_box_create_store ();
	gtk_combo_box_set_model (GTK_COMBO_BOX (combo_box), GTK_TREE_MODEL (store));
        g_object_unref (store);
	...
	gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (combo_box), renderer,
        ...
        g_signal_connect (combo_box, "changed", (GCallback) mdu_util_part_table_type_combo_box_changed, NULL);

        return combo_box;
}

gboolean
mdu_util_part_table_type_combo_box_select (GtkWidget *combo_box, const char *part_table_type)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
        gboolean ret;
...
char *
mdu_util_part_table_type_combo_box_get_selected (GtkWidget *combo_box)
{
        GtkTreeModel *model;
        GtkTreeIter iter;
...
GdkPixbuf *
mdu_util_get_pixbuf_for_presentable (MduPresentable *presentable, GtkIconSize size)
{
        ...
        return mdu_util_get_pixbuf_for_presentable_at_pixel_size (presentable, icon_height);
}

GdkPixbuf *
mdu_util_get_pixbuf_for_presentable_at_pixel_size (MduPresentable *presentable, gint pixel_size)
{
        GIcon *icon;
        GdkPixbuf *pixbuf;
...
void
mdu_util_get_mix_color (GtkWidget    *widget,
                        GtkStateType  state,
                        gchar        *color_buf,
