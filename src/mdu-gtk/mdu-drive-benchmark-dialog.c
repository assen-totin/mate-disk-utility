...
#include "mdu-drive-benchmark-dialog.h"
...
benchmark_data_print (BenchmarkData *data)
{
        ...
        s = mdu_util_get_size_for_display (data->disk_size, FALSE, TRUE);
        g_print ("Disk size: %s\n", s);
        g_free (s);
...
benchmark_data_from_dbus (MduDevice    *device,
                          ...
                          GPtrArray    *access_time_results)
{
        ...
        data->disk_size = mdu_device_get_size (device);
...
static BenchmarkData *
benchmark_data_load (MduDevice   *device,
                     GError     **error)
{
        BenchmarkData *data;
        gchar *filename;
...
struct MduDriveBenchmarkDialogPrivate
{
        ...
        MduDetailsElement *read_min_element;
        MduDetailsElement *write_min_element;
        MduDetailsElement *read_max_element;
        MduDetailsElement *write_max_element;
        MduDetailsElement *read_avg_element;
        MduDetailsElement *write_avg_element;
        MduDetailsElement *updated_element;
        MduDetailsElement *access_avg_element;
};

/* ---------------------------------------------------------------------------------------------------- */

G_DEFINE_TYPE (MduDriveBenchmarkDialog, mdu_drive_benchmark_dialog, MDU_TYPE_DIALOG)

static gdouble is_benchmarking (MduDriveBenchmarkDialog *dialog,
...
static void update_dialog (MduDriveBenchmarkDialog *dialog);
static void on_device_changed (MduDevice *device, gpointer user_data);
static void on_device_job_changed (MduDevice *device, gpointer user_data);

static void
mdu_drive_benchmark_dialog_finalize (GObject *object)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (object);
        ...
        if (G_OBJECT_CLASS (mdu_drive_benchmark_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_drive_benchmark_dialog_parent_class)->finalize (object);
}

/* ---------------------------------------------------------------------------------------------------- */

static void
benchmark_cb (MduDevice    *device,
              GPtrArray    *read_transfer_rate_results,
              GPtrArray    *write_transfer_rate_results,
              GPtrArray    *access_time_results,
              GError       *error,
              gpointer      user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
        GError *local_error;
        GtkAllocation allocation;

        if (error != NULL) {
                if (!(error->domain == MDU_ERROR && error->code == MDU_ERROR_CANCELLED)) {
                        GtkWidget *error_dialog;
                        error_dialog = mdu_error_dialog_new_for_drive (GTK_WINDOW (dialog),
                                                                       device,
...
static void
on_run_benchmark_clicked (MduButtonElement *button_element,
                          gpointer          user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
        const gchar *options[1] = {NULL};

        mdu_device_op_drive_benchmark (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                       FALSE,
                                       options,
                                       benchmark_cb,
                                       g_object_ref (dialog));
...
static void
on_run_write_benchmark_clicked (MduButtonElement *button_element,
                                gpointer          user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
        const gchar *options[1] = {NULL};
        ...
        confirmation_dialog = mdu_confirmation_dialog_new_for_drive (GTK_WINDOW (dialog),
                                                                     mdu_dialog_get_device (MDU_DIALOG (dialog)),
...
static void
cancel_job_cb (MduDevice  *device,
               GError     *error,
               gpointer    user_data)
{
...
static void
on_updated_element_activated (MduDetailsElement    *element,
                              const gchar          *uri,
                              gpointer              user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
...
static void
mdu_drive_benchmark_dialog_constructed (GObject *object)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (object);
        ...
        dialog->priv->device_changed_signal_handler_id = g_signal_connect (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                                                           "changed",
                                                                           G_CALLBACK (on_device_changed),
                                                                           dialog);
        dialog->priv->device_job_changed_signal_handler_id = g_signal_connect (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                                                               "job-changed",
        ...
        element = mdu_details_element_new (_("Minimum Read Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->read_min_element = element;

        element = mdu_details_element_new (_("Minimum Write Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->write_min_element = element;

        element = mdu_details_element_new (_("Maximum Read Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->read_max_element = element;

        element = mdu_details_element_new (_("Maximum Write Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->write_max_element = element;

        element = mdu_details_element_new (_("Average Read Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->read_avg_element = element;

        element = mdu_details_element_new (_("Average Write Rate:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        dialog->priv->write_avg_element = element;

        element = mdu_details_element_new (_("Last Benchmark:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        ...
        element = mdu_details_element_new (_("Average Access Time:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        ...
        button_element = mdu_button_element_new ("gtk-execute",
        ...
        error = NULL;
        dialog->priv->benchmark_data = benchmark_data_load (mdu_dialog_get_device (MDU_DIALOG (dialog)), &error);
        if (dialog->priv->benchmark_data == NULL) {
        ...
        if (G_OBJECT_CLASS (mdu_drive_benchmark_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_drive_benchmark_dialog_parent_class)->constructed (object);
}

static void
mdu_drive_benchmark_dialog_class_init (MduDriveBenchmarkDialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduDriveBenchmarkDialogPrivate));

        object_class->constructed  = mdu_drive_benchmark_dialog_constructed;
        object_class->finalize     = mdu_drive_benchmark_dialog_finalize;
}

static void
mdu_drive_benchmark_dialog_init (MduDriveBenchmarkDialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog, MDU_TYPE_DRIVE_BENCHMARK_DIALOG, MduDriveBenchmarkDialogPrivate);
}

GtkWidget *
mdu_drive_benchmark_dialog_new (GtkWindow *parent,
                                MduDrive  *drive)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_DRIVE_BENCHMARK_DIALOG,
                              ...
                              gpointer        user_data)
{
        ...
        size = mdu_device_get_size (mdu_dialog_get_device (MDU_DIALOG (dialog)));
...
static gdouble
is_benchmarking (MduDriveBenchmarkDialog *dialog,
                 gdouble                 *out_progress)
{
        MduDevice *d;
        gboolean ret;
        ...
        d = mdu_dialog_get_device (MDU_DIALOG (dialog));
        if (!mdu_device_job_in_progress (d))
                goto out;

        if (g_strcmp0 (mdu_device_job_get_id (d), "DriveBenchmark") != 0)
                goto out;

        if (out_progress != NULL)
                *out_progress = mdu_device_job_get_percentage (d);
...
static void
update_dialog (MduDriveBenchmarkDialog *dialog)
{
        gdouble progress;
        ...
        /* benchmark data */
        if (dialog->priv->benchmark_data == NULL) {
                mdu_details_element_set_text (dialog->priv->read_min_element, "–");
                mdu_details_element_set_text (dialog->priv->read_max_element, "–");
                mdu_details_element_set_text (dialog->priv->read_avg_element, "–");
                mdu_details_element_set_text (dialog->priv->write_min_element, "–");
                mdu_details_element_set_text (dialog->priv->write_max_element, "–");
                mdu_details_element_set_text (dialog->priv->write_avg_element, "–");
                /* Translators: This is used for the "Last Benchmark" element when we don't have benchmark data */
                mdu_details_element_set_text (dialog->priv->updated_element, _("Never"));
                mdu_details_element_set_text (dialog->priv->access_avg_element, "–");
                ...
                s = mdu_util_get_speed_for_display (read_min);
                mdu_details_element_set_text (dialog->priv->read_min_element, s);
                g_free (s);
                s = mdu_util_get_speed_for_display (read_max);
                mdu_details_element_set_text (dialog->priv->read_max_element, s);
                g_free (s);
                s = mdu_util_get_speed_for_display (read_avg);
                mdu_details_element_set_text (dialog->priv->read_avg_element, s);
                g_free (s);
                if (dialog->priv->benchmark_data->write_transfer_rate_samples->len > 0) {
                        s = mdu_util_get_speed_for_display (write_min);
                        mdu_details_element_set_text (dialog->priv->write_min_element, s);
                        g_free (s);
                        s = mdu_util_get_speed_for_display (write_max);
                        mdu_details_element_set_text (dialog->priv->write_max_element, s);
                        g_free (s);
                        s = mdu_util_get_speed_for_display (write_avg);
                        mdu_details_element_set_text (dialog->priv->write_avg_element, s);
                        g_free (s);
                } else {
                        mdu_details_element_set_text (dialog->priv->write_min_element, "–");
                        mdu_details_element_set_text (dialog->priv->write_max_element, "–");
                ...
                mdu_details_element_set_text (dialog->priv->access_avg_element, s);
                g_free (s);

                mdu_details_element_set_time (dialog->priv->updated_element,
        ...
        if (is_benchmarking (dialog, &progress)) {
                ...
                if (fraction > 1.0)
                        fraction = 1.0;
                mdu_details_element_set_progress (dialog->priv->updated_element, fraction);
                mdu_details_element_set_time (dialog->priv->updated_element, 0);
                mdu_details_element_set_text (dialog->priv->updated_element, NULL);
                mdu_details_element_set_action_text (dialog->priv->updated_element,
                                                     /* Translators: Text used in the hyperlink in the status
                                                      * table to cancel a self-test */
                                                     _("Cancel"));
                mdu_details_element_set_action_tooltip (dialog->priv->updated_element,
                                                        /* Translators: Tooptip for the "Cancel" hyperlink */
                                                        _("Cancels the currently running benchmark"));
        } else {
                mdu_details_element_set_progress (dialog->priv->updated_element, -1.0);
                mdu_details_element_set_text (dialog->priv->updated_element, NULL);
                mdu_details_element_set_action_text (dialog->priv->updated_element, NULL);
                mdu_details_element_set_action_tooltip (dialog->priv->updated_element, NULL);
        }
...
static void
on_device_changed (MduDevice *device,
                   gpointer   user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
...
static void
on_device_job_changed (MduDevice *device,
                       gpointer   user_data)
{
        MduDriveBenchmarkDialog *dialog = MDU_DRIVE_BENCHMARK_DIALOG (user_data);
