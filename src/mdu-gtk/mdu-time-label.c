...
G_DEFINE_TYPE (MduTimeLabel, mdu_time_label, GTK_TYPE_LABEL)
...
static void do_update (MduTimeLabel *time_label);

static gboolean
timeout_func (MduTimeLabel *time_label)
{
...
static void
do_update (MduTimeLabel *time_label)
{
        char *s;
        GTimeVal now;
        int age;
...
static void
mdu_time_label_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
        MduTimeLabel *time_label = MDU_TIME_LABEL (object);
        GTimeVal *time_val;
...
static void
mdu_time_label_get_property (GObject     *object,
                             guint        prop_id,
                             GValue      *value,
                             GParamSpec  *pspec)
{
...
static void
mdu_time_label_finalize (MduTimeLabel *time_label)
{
        if (time_label->priv->update_timer_id > 0)
                g_source_remove (time_label->priv->update_timer_id);
...
static void
mdu_time_label_class_init (MduTimeLabelClass *klass)
{
        ...
        obj_class->finalize = (GObjectFinalizeFunc) mdu_time_label_finalize;
        obj_class->set_property = mdu_time_label_set_property;
        obj_class->get_property = mdu_time_label_get_property;

        g_type_class_add_private (klass, sizeof (MduTimeLabelPrivate));

        boxed_type = g_boxed_type_register_static ("MduTimeLabelBoxedGTimeVal",
...
static void
mdu_time_label_init (MduTimeLabel *time_label)
{
        time_label->priv = G_TYPE_INSTANCE_GET_PRIVATE (time_label, MDU_TYPE_TIME_LABEL, MduTimeLabelPrivate);
}

GtkWidget *
mdu_time_label_new (GTimeVal *time)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_TIME_LABEL, "time", time, NULL));
}

void
mdu_time_label_set_time (MduTimeLabel *time_label, GTimeVal     *time)
{
