...
#include "mdu-time-label.h"
#include "mdu-ata-smart-dialog.h"
#include "mdu-spinner.h"
#include "mdu-pool-tree-model.h"
#include "mdu-details-table.h"
#include "mdu-details-element.h"

/* ---------------------------------------------------------------------------------------------------- */

struct MduAtaSmartDialogPrivate
{
        guint64 last_updated;
        gulong device_changed_signal_handler_id;
        gulong device_job_changed_signal_handler_id;

        MduDetailsElement *updated_element;
        MduDetailsElement *self_test_element;
        MduDetailsElement *powered_on_element;
        MduDetailsElement *power_cycles_element;
        MduDetailsElement *temperature_element;
        MduDetailsElement *bad_sectors_element;
        MduDetailsElement *self_assessment_element;
        MduDetailsElement *overall_assessment_element;

        MduButtonElement *refresh_button;
        MduButtonElement *self_test_button;
        MduButtonElement *cancel_self_test_button;
...
static gboolean is_self_test_running (MduDevice *device,
                                      SkSmartSelfTest *out_test_type);

/* ---------------------------------------------------------------------------------------------------- */

#define _MDU_TYPE_SK_ATTR (_mdu_type_sk_attr_get_type ())

static SkSmartAttributeParsedData *
_mdu_sk_attr_copy (const SkSmartAttributeParsedData *instance)
{
...
static void
_mdu_sk_attr_free (SkSmartAttributeParsedData *instance)
{
...
static GType
_mdu_type_sk_attr_get_type (void)
{
        static volatile gsize type_volatile = 0;

        if (g_once_init_enter (&type_volatile)) {
                GType type = g_boxed_type_register_static (
                                                           g_intern_static_string ("_MduSkAttr"),
                                                           (GBoxedCopyFunc) _mdu_sk_attr_copy,
                                                           (GBoxedFreeFunc) _mdu_sk_attr_free);
...
G_DEFINE_TYPE (MduAtaSmartDialog, mdu_ata_smart_dialog, MDU_TYPE_DIALOG)

static void update_dialog (MduAtaSmartDialog *dialog);
...
static gboolean get_ata_smart_no_warn (MduDevice *device);
static gboolean set_ata_smart_no_warn (MduDevice *device,
                                       gboolean   no_warn);

static void
mdu_ata_smart_dialog_finalize (GObject *object)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (object);

        g_signal_handler_disconnect (mdu_dialog_get_device (MDU_DIALOG (dialog)), dialog->priv->device_changed_signal_handler_id);
        g_signal_handler_disconnect (mdu_dialog_get_device (MDU_DIALOG (dialog)), dialog->priv->device_job_changed_signal_handler_id);
        g_object_unref (dialog->priv->attr_list_store);

        if (G_OBJECT_CLASS (mdu_ata_smart_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_ata_smart_dialog_parent_class)->finalize (object);
}
...
selection_changed (GtkTreeSelection *tree_selection,
                   gpointer          user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);
        GtkTreeIter iter;
        gchar *attr_name;
                    ...
                    gpointer         user_data)
{
        ...
        if (a->warn) {
                ...
                mdu_util_get_mix_color (GTK_WIDGET (dialog->priv->tree_view), state, color, sizeof (color));
                   ...
                   gpointer         user_data)
{
        ...
        if (a->warn) {
                icon_name = "mdu-smart-failing";
        } else {
                if (a->good_now_valid)
                        icon_name = "mdu-smart-healthy";
                else
                        icon_name = "mdu-smart-unknown";
        }
            ...
            gpointer    user_data)
{
        ...
        if (error != NULL) {
                ...
                error_dialog = mdu_error_dialog_new (GTK_WINDOW (gtk_widget_get_toplevel (GTK_WIDGET (dialog))),
                                                     mdu_dialog_get_presentable (MDU_DIALOG (dialog)),
...
static void
on_refresh_button_clicked (MduButtonElement *element,
                           gpointer          user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);

        mdu_device_drive_ata_smart_refresh_data (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                                 refresh_cb,
                                                 dialog);
...
static void
cancel_self_test_cb (MduDevice  *device,
                     GError     *error,
                     gpointer    user_data)
{
        /* TODO: maybe show error dialog */
...
static void
on_cancel_self_test_button_clicked (MduButtonElement *button_element,
                                    gpointer          user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);

        if (is_self_test_running (mdu_dialog_get_device (MDU_DIALOG (dialog)), NULL)) {
                mdu_device_op_cancel_job (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                          cancel_self_test_cb,
                                          dialog);
        } else {
...
static void
run_self_test_cb (MduDevice  *device,
                  GError     *error,
                  gpointer    user_data)
{
        /* TODO: maybe show error dialog */
...
static void
on_self_test_button_clicked (MduButtonElement *button_element,
                             gpointer          user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);
	...
	gtk_box_pack_start (GTK_BOX (main_vbox), GTK_WIDGET (radio2), FALSE, FALSE, 0);
        ...
        mdu_device_op_drive_ata_smart_initiate_selftest (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                                         test,
                                                         run_self_test_cb,
                                                         dialog);
                                 ...
                                 gpointer         user_data)
{
        ...
        if (mdu_dialog_get_device (MDU_DIALOG (dialog)) == NULL)
                goto out;

        if (get_ata_smart_no_warn (mdu_dialog_get_device (MDU_DIALOG (dialog))) != is_active) {
                set_ata_smart_no_warn (mdu_dialog_get_device (MDU_DIALOG (dialog)), is_active);
                update_dialog (dialog);
        }
...
static void
mdu_ata_smart_dialog_constructed (GObject *object)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (object);
        GtkWidget *content_area;
        GtkWidget *align;
        ...
        dialog->priv->device_changed_signal_handler_id =
                g_signal_connect (mdu_dialog_get_device (MDU_DIALOG (dialog)),
                                  "changed",
        ...
        element = mdu_details_element_new (_("Updated:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Self-tests:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Powered On:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Power Cycles:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Temperature:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Bad Sectors:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Self Assessment:"),
                                           NULL,
        ...
        /* Translators: Item name in the status table */
        element = mdu_details_element_new (_("Overall Assessment:"),
                                           NULL,
        ...
        button_element = mdu_button_element_new ("gtk-refresh",
        ...
        button_element = mdu_button_element_new ("gtk-cancel",
                                                 /* Translators: Button name */
                                                 _("_Cancel Self-test"),
                                                 /* Translators: Button details*/
        ...
        dialog->priv->attr_list_store = gtk_list_store_new (N_COLUMNS,
                                                            ...
                                                            _MDU_TYPE_SK_ATTR); /* SkSmartAttributeParsedData pointer */
        ...
        if (G_OBJECT_CLASS (mdu_ata_smart_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_ata_smart_dialog_parent_class)->constructed (object);
}

static void
mdu_ata_smart_dialog_class_init (MduAtaSmartDialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduAtaSmartDialogPrivate));

        object_class->constructed  = mdu_ata_smart_dialog_constructed;
        object_class->finalize     = mdu_ata_smart_dialog_finalize;
}

static void
mdu_ata_smart_dialog_init (MduAtaSmartDialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog, MDU_TYPE_ATA_SMART_DIALOG, MduAtaSmartDialogPrivate);
}

GtkWidget *
mdu_ata_smart_dialog_new (GtkWindow *parent,
                          MduDrive  *drive)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_ATA_SMART_DIALOG,
                                         "transient-for", parent,
                                         "presentable", MDU_PRESENTABLE (drive),
                                         NULL));
}
...
static gboolean
is_self_test_running (MduDevice *device,
                      SkSmartSelfTest *out_test_type)
{
        gboolean ret;
        ...
        if (!mdu_device_job_in_progress (device))
                goto out;

        if (g_strcmp0 (mdu_device_job_get_id (device), "DriveAtaSmartSelftestShort") == 0) {
                ret = TRUE;
                if (out_test_type != NULL)
                        *out_test_type = SK_SMART_SELF_TEST_SHORT;
        } else if (g_strcmp0 (mdu_device_job_get_id (device), "DriveAtaSmartSelftestExtended") == 0) {
                ret = TRUE;
                if (out_test_type != NULL)
                        *out_test_type = SK_SMART_SELF_TEST_EXTENDED;
        } else if (g_strcmp0 (mdu_device_job_get_id (device), "DriveAtaSmartSelftestConveyance") == 0) {
                ret = TRUE;
                if (out_test_type != NULL)
...
static gboolean
get_ata_smart_no_warn (MduDevice *device)
{
        gboolean ret;
        gchar *path;
        gchar *disk_id;
        ...
        disk_id = g_strdup_printf ("%s-%s-%s-%s",
                                   mdu_device_drive_get_vendor (device),
                                   mdu_device_drive_get_model (device),
                                   mdu_device_drive_get_revision (device),
                                   mdu_device_drive_get_serial (device));
...
static gboolean
set_ata_smart_no_warn (MduDevice *device,
                       gboolean   no_warn)
{
        gboolean ret;
        gchar *path;
...
static void
update_dialog (MduAtaSmartDialog *dialog)
{
        ...
        if (!mdu_device_drive_ata_smart_get_is_available (device)) {
                /* Translators: Shown in the "Overall Assessment" item in the status table
        ...
        blob = mdu_device_drive_ata_smart_get_blob (device, &blob_size);
        if (blob == NULL) {
                /* Translators: Shown in the "Overall Assessment" item in the status table
        ...
        dialog->priv->last_updated = updated = mdu_device_drive_ata_smart_get_time_collected (device);

        s = mdu_util_ata_smart_status_to_desc (mdu_device_drive_ata_smart_get_status (device),
                                               &highlight,
                                               &action_text,
                                               &status_icon);
        if (highlight) {
 ...
 has_data:
        ...
        if (status_icon == NULL)
                status_icon = g_themed_icon_new ("mdu-smart-unknown");
        mdu_details_element_set_icon (dialog->priv->overall_assessment_element, status_icon);

        mdu_details_element_set_text (dialog->priv->self_assessment_element,
                                      self_assessment_text != NULL ? self_assessment_text : "-");
        mdu_details_element_set_text (dialog->priv->overall_assessment_element,
                                      overall_assessment_text != NULL ? overall_assessment_text : "-");
        mdu_details_element_set_text (dialog->priv->bad_sectors_element,
                                      bad_sectors_text != NULL ? bad_sectors_text : "-");
        mdu_details_element_set_text (dialog->priv->powered_on_element,
                                      powered_on_text != NULL ? powered_on_text : "-");
        mdu_details_element_set_text (dialog->priv->power_cycles_element,
                                      power_cycles_text != NULL ? power_cycles_text : "-");
        mdu_details_element_set_text (dialog->priv->temperature_element,
                                      temperature_text != NULL ? temperature_text : "-");
        if (dialog->priv->is_updating) {
                mdu_details_element_set_is_spinning (dialog->priv->updated_element, TRUE);
                mdu_details_element_set_text (dialog->priv->updated_element, "Updating");
                mdu_details_element_set_time (dialog->priv->updated_element, 0);
        } else {
                mdu_details_element_set_is_spinning (dialog->priv->updated_element, FALSE);
                if (updated > 0) {
                        mdu_details_element_set_time (dialog->priv->updated_element, updated);
                        mdu_details_element_set_text (dialog->priv->updated_element, NULL);
                } else {
                        mdu_details_element_set_time (dialog->priv->updated_element, 0);
                        mdu_details_element_set_text (dialog->priv->updated_element, "-");
                }
        }
        ...
        if (device != NULL && is_self_test_running (device, &test_type)) {
                ...
                mdu_button_element_set_visible (dialog->priv->self_test_button, FALSE);

                if (mdu_device_job_is_cancellable (device)) {
                        mdu_button_element_set_visible (dialog->priv->cancel_self_test_button, TRUE);
                } else {
                        mdu_button_element_set_visible (dialog->priv->cancel_self_test_button, FALSE);
                }
                mdu_details_element_set_text (dialog->priv->self_test_element, NULL);

                fraction = mdu_device_job_get_percentage (device) / 100.0;
                if (fraction < 0.0)
                        fraction = 0.0;
                if (fraction > 1.0)
                        fraction = 1.0;

                mdu_details_element_set_progress (dialog->priv->self_test_element, fraction);
        } else {
                mdu_details_element_set_progress (dialog->priv->self_test_element, -1.0);
                mdu_details_element_set_text (dialog->priv->self_test_element,
                ...
                if (device != NULL) {
                        ...
                        mdu_button_element_set_visible (dialog->priv->self_test_button, TRUE);
                } else {
                        mdu_button_element_set_visible (dialog->priv->self_test_button, FALSE);
                }
        }
...
static void
device_changed (MduDevice *device,
                gpointer   user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);

        if (mdu_device_drive_ata_smart_get_time_collected (device) != dialog->priv->last_updated) {
                update_dialog (dialog);
        }
...
static void
device_job_changed (MduDevice *device,
                    gpointer   user_data)
{
        MduAtaSmartDialog *dialog = MDU_ATA_SMART_DIALOG (user_data);
