...
struct GridElement
{
        ...
        gdouble size_ratio;
        MduPresentable *presentable; /* if NULL, it means no media is available */
...
G_DEFINE_TYPE (MduVolumeGrid, mdu_volume_grid, GTK_TYPE_DRAWING_AREA)

static guint get_depth (GList *elements);

static guint get_num_elements_for_slice (GList *elements);

static void recompute_grid (MduVolumeGrid *grid);

static void recompute_size (MduVolumeGrid *grid,
                            guint          width,
                            guint          height);

static GridElement *find_element_for_presentable (MduVolumeGrid *grid,
                                                  MduPresentable *presentable);

static GridElement *find_element_for_position (MduVolumeGrid *grid,
                                               guint x,
                                               guint y);

static gboolean mdu_volume_grid_expose_event (GtkWidget           *widget,
...
static void on_presentable_changed      (MduPool        *pool,
                                         MduPresentable *p,
                                         gpointer        user_data);
static void on_presentable_job_changed (MduPool        *pool,
                                        MduPresentable *p,
                                        gpointer        user_data);

static void
mdu_volume_grid_finalize (GObject *object)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (object);
        ...
        if (G_OBJECT_CLASS (mdu_volume_grid_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_volume_grid_parent_class)->finalize (object);
}

static void
mdu_volume_grid_get_property (GObject    *object,
                              guint       property_id,
                              GValue     *value,
                              GParamSpec *pspec)
{
...
static void
mdu_volume_grid_set_property (GObject      *object,
                              guint         property_id,
                              const GValue *value,
                              GParamSpec   *pspec)
{
        ...
        switch (property_id) {
        case PROP_DRIVE:
                grid->priv->drive = MDU_DRIVE (g_value_dup_object (value));
                grid->priv->pool = mdu_presentable_get_pool (MDU_PRESENTABLE (grid->priv->drive));
                grid->priv->device = mdu_presentable_get_device (MDU_PRESENTABLE (grid->priv->drive));
                break;
...
static void
mdu_volume_grid_constructed (GObject *object)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (object);
        ...
        if (G_OBJECT_CLASS (mdu_volume_grid_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_volume_grid_parent_class)->constructed (object);
}
...
static gboolean
mdu_volume_grid_key_press_event (GtkWidget      *widget,
                                 GdkEventKey    *event)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (widget);
        gboolean handled;
        GridElement *target;
...
static gboolean
mdu_volume_grid_button_press_event (GtkWidget      *widget,
...
static void
mdu_volume_grid_realize (GtkWidget *widget)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (widget);
        GdkWindow *window;
...
static void
mdu_volume_grid_size_request (GtkWidget      *widget,
...
static void
mdu_volume_grid_class_init (MduVolumeGridClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);
        GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduVolumeGridPrivate));

        object_class->get_property = mdu_volume_grid_get_property;
        object_class->set_property = mdu_volume_grid_set_property;
        object_class->constructed  = mdu_volume_grid_constructed;
        object_class->finalize     = mdu_volume_grid_finalize;

        widget_class->realize            = mdu_volume_grid_realize;
        widget_class->key_press_event    = mdu_volume_grid_key_press_event;
        widget_class->button_press_event = mdu_volume_grid_button_press_event;
        widget_class->size_request       = mdu_volume_grid_size_request;
        widget_class->expose_event       = mdu_volume_grid_expose_event;
        ...
        signals[CHANGED_SIGNAL] = g_signal_new ("changed",
                                                MDU_TYPE_VOLUME_GRID,
                                                G_SIGNAL_RUN_LAST,
                                                G_STRUCT_OFFSET (MduVolumeGridClass, changed),
                                                NULL,
                                                NULL,
...
static void
mdu_volume_grid_init (MduVolumeGrid *grid)
{
        grid->priv = G_TYPE_INSTANCE_GET_PRIVATE (grid, MDU_TYPE_VOLUME_GRID, MduVolumeGridPrivate);

        gtk_widget_set_can_focus (GTK_WIDGET (grid), TRUE);
}

GtkWidget *
mdu_volume_grid_new (MduDrive *drive)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_VOLUME_GRID,
                                         "drive", drive,
                                         NULL));
}

static gint
presentable_sort_offset (MduPresentable *a, MduPresentable *b)
{
        guint64 oa, ob;

        oa = mdu_presentable_get_offset (a);
        ob = mdu_presentable_get_offset (b);
...
static void
recompute_size_for_slice (MduPresentable *presentable,
                          GList          *elements,
                          guint           width,
                          guint           height,
                          guint           total_width,
...
static void
recompute_size (MduVolumeGrid *grid,
                guint          width,
                guint          height)
{
        recompute_size_for_slice (MDU_PRESENTABLE (grid->priv->drive),
                                  grid->priv->elements,
                                  width,
                                  height,
                                  width,
...
static void
add_luks_holders (MduVolumeGrid *grid,
                  GridElement   *element)
{
        MduDevice *d;
        MduDevice *dholder;
        MduPresentable *pholder;
        const gchar *holder;
        ...
        d = mdu_presentable_get_device (element->presentable);
        if (d == NULL)
                goto out;

        if (g_strcmp0 (mdu_device_id_get_usage (d), "crypto") != 0)
                goto out;

        holder = mdu_device_luks_get_holder (d);
        if (holder == NULL || g_strcmp0 (holder, "/") == 0)
                goto out;

        dholder = mdu_pool_get_by_object_path (grid->priv->pool, holder);
        if (dholder == NULL)
                goto out;

        pholder = mdu_pool_get_volume_by_device (grid->priv->pool, dholder);
        if (pholder == NULL)
                goto out;
        ...
        //g_debug ("added holder %s for %s", mdu_device_get_device_file (dholder), mdu_device_get_device_file (d));
...
static void
recompute_grid (MduVolumeGrid *grid)
{
        MduPresentable *cur_selected_presentable;
        MduPresentable *cur_focused_presentable;
        ...
        size = mdu_presentable_get_size (MDU_PRESENTABLE (grid->priv->drive));
        enclosed_partitions = mdu_pool_get_enclosed_presentables (grid->priv->pool,
                                                                  MDU_PRESENTABLE (grid->priv->drive));
        ...
        for (l = enclosed_partitions; l != NULL; l = l->next) {
                ...
                ed = mdu_presentable_get_device (ep);

                psize = mdu_presentable_get_size (ep);
                ...
                if (ed != NULL && mdu_device_is_partition (ed) &&
                    (g_strcmp0 (mdu_device_partition_get_type (ed), "0x05") == 0 ||
                     g_strcmp0 (mdu_device_partition_get_type (ed), "0x0f") == 0 ||
                     g_strcmp0 (mdu_device_partition_get_type (ed), "0x85") == 0)) {
                        GList *enclosed_logical_partitions;
                        GList *ll;
                        GridElement *logical_element;
                        GridElement *logical_prev_element;

                        enclosed_logical_partitions = mdu_pool_get_enclosed_presentables (grid->priv->pool, ep);
                        ...
                        for (ll = enclosed_logical_partitions; ll != NULL; ll = ll->next) {
                                MduPresentable *logical_ep = MDU_PRESENTABLE (ll->data);
                                guint64 lsize;

                                lsize = mdu_presentable_get_size (logical_ep);
...
static gboolean
render_element (MduVolumeGrid *grid,
                cairo_t       *cr,
                GridElement   *element,
                ...
                gboolean       is_grid_focused)
{
        ...
        if (element->presentable == NULL) { /* no media available */
                ...
                if (MDU_IS_LINUX_MD_DRIVE (grid->priv->drive)) {
                ...
                if (d != NULL && mdu_device_job_in_progress (d))
                        render_job_in_progress = TRUE;

                str = g_string_new (NULL);

                size = mdu_presentable_get_size (element->presentable);
                size_str = mdu_util_get_size_for_display (size,
                                                          FALSE,
                                                          FALSE);

                if (d != NULL && g_strcmp0 (mdu_device_id_get_usage (d), "filesystem") == 0) {
                        gchar *fstype_str;
                        gchar *size_str;

                        fstype_str = mdu_util_get_fstype_for_display (mdu_device_id_get_type (d),
                                                                      mdu_device_id_get_version (d),
                                                                      FALSE);
                        size_str = mdu_util_get_size_for_display (mdu_device_get_size (d),
                                                                  FALSE,
                                                                  FALSE);

                        if (strlen (mdu_device_id_get_label (d)) > 0)
                                g_string_append_printf (str, "%s\n", mdu_device_id_get_label (d));
                        g_string_append_printf (str, "%s %s", size_str, fstype_str);

                        g_free (fstype_str);
                } else if (d != NULL && mdu_device_is_partition (d) &&
                ...
                } else if (d != NULL && g_strcmp0 (mdu_device_id_get_usage (d), "crypto") == 0) {
                ...
                } else if (d != NULL && g_strcmp0 (mdu_device_id_get_usage (d), "raid") == 0 &&
                           mdu_device_is_linux_md_component (d)) {

                        g_string_append_printf (str,
                                                "%s\n%s",
                                                /* Translators: shown in the grid for an RAID Component */
                                                _("RAID Component"),
                                                mdu_device_linux_md_component_get_name (d));

                } else if (!mdu_presentable_is_allocated (element->presentable)) {
                ...
                } else if (!mdu_presentable_is_recognized (element->presentable)) {
                        ...
                        /* FALLBACK */
                        presentable_name = mdu_presentable_get_name (element->presentable);
                        if (size > 0) {
...
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (data);
...
static gboolean
render_slice (MduVolumeGrid *grid,
              cairo_t       *cr,
              GList         *elements)
{
        GList *l;
...
static gboolean
mdu_volume_grid_expose_event (GtkWidget           *widget,
                              GdkEventExpose      *event)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (widget);
        GtkAllocation allocation;
...
static GridElement *
find_element_for_presentable (MduVolumeGrid *grid,
                              MduPresentable *presentable)
{
...
static GridElement *
find_element_for_position (MduVolumeGrid *grid,
                           guint x,
                           guint y)
{
...
MduPresentable *
mdu_volume_grid_get_selected (MduVolumeGrid *grid)
{
        MduPresentable *ret;

        g_return_val_if_fail (MDU_IS_VOLUME_GRID (grid), NULL);
...
static void
maybe_recompute (MduVolumeGrid  *grid,
                 MduPresentable *p)
{
        gboolean recompute;

        recompute = FALSE;
        if (p == MDU_PRESENTABLE (grid->priv->drive) ||
            mdu_presentable_encloses (MDU_PRESENTABLE (grid->priv->drive), p) ||
...
on_presentable_added (MduPool        *pool,
                      ...
                      gpointer        user_data)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (user_data);
...
static void
on_presentable_job_changed (MduPool        *pool,
                            MduPresentable *p,
                            gpointer        user_data)
{
        MduVolumeGrid *grid = MDU_VOLUME_GRID (user_data);
...
gboolean
mdu_volume_grid_select (MduVolumeGrid       *grid,
                        MduPresentable      *volume)
{
        GridElement *element;
        gboolean ret;

        g_return_val_if_fail (MDU_IS_VOLUME_GRID (grid), FALSE);
