...
#include <mdu/mdu-marshal.h>

#include "mdu-disk-selection-widget.h"
...
struct MduDiskSelectionWidgetPrivate
{
        MduPool *pool;
        ...
        /* A list of MduDrive objects that are selected */
...
static void mdu_disk_selection_widget_constructed (GObject *object);

static void emit_changed (MduDiskSelectionWidget *widget);
...
G_DEFINE_TYPE (MduDiskSelectionWidget, mdu_disk_selection_widget, GTK_TYPE_VBOX)

static void
mdu_disk_selection_widget_finalize (GObject *object)
{
        MduDiskSelectionWidget *widget = MDU_DISK_SELECTION_WIDGET (object);
        ...
        if (G_OBJECT_CLASS (mdu_disk_selection_widget_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_disk_selection_widget_parent_class)->finalize (object);
}

static void
mdu_disk_selection_widget_get_property (GObject    *object,
                                         guint       property_id,
                                         GValue     *value,
                                         GParamSpec *pspec)
{
        ...
        case PROP_SELECTED_DRIVES:
                p = mdu_disk_selection_widget_get_selected_drives (widget);
                g_value_set_boxed (value, p);
                g_ptr_array_unref (p);
                break;

        case PROP_COMPONENT_SIZE:
                g_value_set_uint64 (value, mdu_disk_selection_widget_get_component_size (widget));
                break;

        case PROP_NUM_AVAILABLE_DISKS:
                g_value_set_uint (value, mdu_disk_selection_widget_get_num_available_disks (widget));
                break;

        case PROP_LARGEST_SEGMENT_FOR_SELECTED:
                g_value_set_uint64 (value, mdu_disk_selection_widget_get_largest_segment_for_selected (widget));
                break;

        case PROP_LARGEST_SEGMENT_FOR_ALL:
                g_value_set_uint64 (value, mdu_disk_selection_widget_get_largest_segment_for_all (widget));
                break;
...
static void
mdu_disk_selection_widget_set_property (GObject      *object,
                                         guint         property_id,
                                         const GValue *value,
                                         GParamSpec   *pspec)
{
        ...
        case PROP_COMPONENT_SIZE:
                mdu_disk_selection_widget_set_component_size (widget, g_value_get_uint64 (value));
                break;
...
static gboolean
mdu_disk_selection_widget_mnemonic_activate (GtkWidget *_widget,
                                             gboolean   group_cycling)
{
        MduDiskSelectionWidget *widget = MDU_DISK_SELECTION_WIDGET (_widget);
        return gtk_widget_mnemonic_activate (widget->priv->tree_view, group_cycling);
}

static void
mdu_disk_selection_widget_class_init (MduDiskSelectionWidgetClass *klass)
{
        ...
        g_type_class_add_private (klass, sizeof (MduDiskSelectionWidgetPrivate));

        gobject_class->get_property = mdu_disk_selection_widget_get_property;
        gobject_class->set_property = mdu_disk_selection_widget_set_property;
        gobject_class->constructed  = mdu_disk_selection_widget_constructed;
        gobject_class->finalize     = mdu_disk_selection_widget_finalize;

        widget_class->mnemonic_activate = mdu_disk_selection_widget_mnemonic_activate;
        ...
        g_object_class_install_property (gobject_class,
                                         ...
                                         g_param_spec_flags ("flags",
                                                             ...
                                                             MDU_TYPE_DISK_SELECTION_WIDGET_FLAGS,
                                                             MDU_DISK_SELECTION_WIDGET_FLAGS_NONE,
                                                             G_PARAM_READABLE |
                                                             G_PARAM_WRITABLE |
                                                             G_PARAM_CONSTRUCT_ONLY |
                                                             G_PARAM_STATIC_NAME |
        ...
        signals[CHANGED_SIGNAL] = g_signal_new ("changed",
                                                ...
                                                G_SIGNAL_RUN_LAST,
                                                G_STRUCT_OFFSET (MduDiskSelectionWidgetClass, changed),
                                                NULL,
                                                NULL,
        ...
        /**
         * MduDiskSelectionWidget::is-drive-ignored:
         * @widget: A #MduDiskSelectionWidget.
         * @drive: A #MduDrive.
         *
        ...
        signals[IS_DRIVE_IGNORED_SIGNAL] = g_signal_new ("is-drive-ignored",
                                                         G_TYPE_FROM_CLASS (klass),
                                                         G_SIGNAL_RUN_LAST,
                                                         G_STRUCT_OFFSET (MduDiskSelectionWidgetClass, is_drive_ignored),
                                                         NULL, /* accumulator function */
                                                         NULL, /* user_data for accumulator function */
                                                         mdu_marshal_STRING__OBJECT,
                                                         G_TYPE_STRING,
                                                         1,
                                                         MDU_TYPE_DRIVE);
...
static void
mdu_disk_selection_widget_init (MduDiskSelectionWidget *widget)
{
        widget->priv = G_TYPE_INSTANCE_GET_PRIVATE (widget,
                                                    MDU_TYPE_DISK_SELECTION_WIDGET,
                                                    MduDiskSelectionWidgetPrivate);
}

GtkWidget *
mdu_disk_selection_widget_new (MduPool                     *pool,
                               MduDiskSelectionWidgetFlags  flags)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_DISK_SELECTION_WIDGET,
                                         "pool", pool,
                                         "flags", flags,
                                         NULL));
}
...
GPtrArray *
mdu_disk_selection_widget_get_selected_drives (MduDiskSelectionWidget  *widget)
{
        GPtrArray *p;
        GList *l;

        g_return_val_if_fail (MDU_IS_DISK_SELECTION_WIDGET (widget), NULL);

        p = g_ptr_array_new_with_free_func (g_object_unref);
        for (l = widget->priv->selected_drives; l != NULL; l = l->next) {
                MduPresentable *drive = MDU_PRESENTABLE (l->data);
...
static gboolean
is_drive_selectable (MduDiskSelectionWidget *widget,
                     MduDrive               *drive,
                     gchar                 **out_reason)
{
        gboolean ret;
        ...
        if (d != NULL && mdu_device_should_ignore (d)) {
        ...
        if (mdu_drive_can_create_volume (drive,
                                         ...
                                         NULL, /* total_free */
                                         NULL)) {
                if (widget->priv->flags & MDU_DISK_SELECTION_WIDGET_FLAGS_ALLOW_DISKS_WITH_INSUFFICIENT_SPACE) {
                        ...
                        if (largest_segment >= widget->priv->component_size) {
                                ...
                                if (largest_segment < SIZE_EPSILON) {
                                       ...
                                       gchar *s1;
                                       gchar *s2;
                                       s1 = mdu_util_get_size_for_display (widget->priv->component_size, FALSE, FALSE);
                                       s2 = mdu_util_get_size_for_display (largest_segment, FALSE, FALSE);
...
static gboolean
drive_is_selected (MduDiskSelectionWidget *widget,
...
static void
drive_remove (MduDiskSelectionWidget *widget,
              MduPresentable         *drive)
{
        GList *l;
...
static void
drive_add (MduDiskSelectionWidget *widget,
           MduPresentable         *drive)
{
...
static void
drive_toggle (MduDiskSelectionWidget *widget,
              MduPresentable         *drive)
{
...
toggle_data_func (GtkCellLayout   *cell_layout,
                  ...
                  gpointer         user_data)
{
        MduDiskSelectionWidget *widget = MDU_DISK_SELECTION_WIDGET (user_data);
        MduPresentable *p;
        gboolean is_visible;
        gboolean is_toggled;
        ...
        gtk_tree_model_get (tree_model,
                            iter,
                            MDU_POOL_TREE_MODEL_COLUMN_PRESENTABLE, &p,
                            -1);

        is_visible = FALSE;
        is_toggled = FALSE;
        if (MDU_IS_DRIVE (p)) {
                is_visible = is_drive_selectable (widget, MDU_DRIVE (p), NULL);
                 ...
                 gpointer               user_data)
{
        ...
        if (! (widget->priv->flags & MDU_DISK_SELECTION_WIDGET_FLAGS_ALLOW_MULTIPLE)) {
                /* untoggle all */
                g_list_foreach (widget->priv->selected_drives, (GFunc) g_object_unref, NULL);
                g_list_free (widget->priv->selected_drives);
                ...
                gpointer         user_data)
{
        ...
        sensitive = TRUE;
        if (p != NULL && MDU_IS_DRIVE (p))
                sensitive = is_drive_selectable (widget, MDU_DRIVE (p), NULL);
                     ...
                     gpointer         user_data)
{
        ...
        mdu_util_get_mix_color (GTK_WIDGET (widget->priv->tree_view), state, color, sizeof (color));
                                ...
                                gpointer      user_data)
{
        ...
        if (MDU_IS_DRIVE (p)) {
                if (is_drive_selectable (data->widget, MDU_DRIVE (p), NULL))
                                   ...
                                   gpointer      user_data)
{
        ...
        if (MDU_IS_DRIVE (p)) {
                ...
                if (mdu_drive_can_create_volume (MDU_DRIVE (p),
                                                 NULL,
                                                 &largest_segment,
                                                 NULL, /* total_free */
                                                 NULL)) {
...
static void
get_sizes (MduDiskSelectionWidget *widget,
           guint                  *out_num_disks,
                 ...
                 gpointer         user_data)
{
        ...
        remaining_size = 0;
        if (d != NULL && mdu_device_is_partition_table (d)) {
                is_partitioned = TRUE;
                num_partitions = mdu_device_partition_table_get_count (d);
        }

        /* TODO: probably want a vfunc for this */
        if (MDU_IS_LINUX_LVM2_VOLUME_GROUP (p))
                is_partitionable = FALSE;

        can_create_volume = mdu_drive_can_create_volume (MDU_DRIVE (p),
                                                         &whole_disk_is_uninitialized,
                                                         &largest_segment,
                                                         &total_free,
                                                         NULL);

        is_selectable = is_drive_selectable (widget, MDU_DRIVE (p), &reason);
        ...
        if (drive_is_selected (widget, p)) {
                strsize = mdu_util_get_size_for_display (widget->priv->component_size, FALSE, FALSE);
                rem_strsize = mdu_util_get_size_for_display (remaining_size, FALSE, FALSE);
                ...
                strsize = mdu_util_get_size_for_display (largest_segment, FALSE, FALSE);
                if (whole_disk_is_uninitialized) {
                        /* Translators: This is shown in the Details column.
...
static void
mdu_disk_selection_widget_constructed (GObject *object)
{
        MduDiskSelectionWidget *widget = MDU_DISK_SELECTION_WIDGET (object);
        GtkWidget *tree_view;
        ...
        model = GTK_TREE_MODEL (mdu_pool_tree_model_new (pools,
                                                         NULL,
                                                         MDU_POOL_TREE_MODEL_FLAGS_NO_VOLUMES /* | MDU_POOL_TREE_MODEL_FLAGS_NO_UNALLOCATABLE_DRIVES */));
        ...
        if (G_OBJECT_CLASS (mdu_disk_selection_widget_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_disk_selection_widget_parent_class)->constructed (object);
}
...
guint64
mdu_disk_selection_widget_get_component_size (MduDiskSelectionWidget *widget)
{
        g_return_val_if_fail (MDU_IS_DISK_SELECTION_WIDGET (widget), 0);
        return widget->priv->component_size;
}

void
mdu_disk_selection_widget_set_component_size (MduDiskSelectionWidget *widget,
                                              guint64                 component_size)
{
        g_return_if_fail (MDU_IS_DISK_SELECTION_WIDGET (widget));
...
guint
mdu_disk_selection_widget_get_num_available_disks (MduDiskSelectionWidget *widget)
{
        guint num_available_disks;
...
guint64
mdu_disk_selection_widget_get_largest_segment_for_selected (MduDiskSelectionWidget *widget)
{
        guint64 largest_segment_for_selected;
...
guint64
mdu_disk_selection_widget_get_largest_segment_for_all (MduDiskSelectionWidget *widget)
{
        guint64 largest_segment_for_all;
...
static void
emit_changed (MduDiskSelectionWidget *widget)
{
