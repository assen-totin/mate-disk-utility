...
#include "mdu-edit-linux-md-dialog.h"
#include "mdu-size-widget.h"

struct MduEditLinuxMdDialogPrivate
{
        gulong changed_id;
        gulong removed_id;
        ...
        MduDetailsElement *component_smart_element;
        MduDetailsElement *component_position_element;
        MduDetailsElement *component_state_element;
        MduDetailsElement *component_device_element;

        MduButtonElement *add_spare_button;
        MduButtonElement *expand_button;
        MduButtonElement *component_attach_button;
        MduButtonElement *component_remove_button;
...
static void mdu_edit_linux_md_dialog_constructed (GObject *object);

static void update_tree (MduEditLinuxMdDialog *dialog);
static void update_details (MduEditLinuxMdDialog *dialog);

static MduDevice *get_selected_component (MduEditLinuxMdDialog *dialog);

G_DEFINE_TYPE (MduEditLinuxMdDialog, mdu_edit_linux_md_dialog, MDU_TYPE_DIALOG)

static void
mdu_edit_linux_md_dialog_finalize (GObject *object)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (object);
        ...
        if (G_OBJECT_CLASS (mdu_edit_linux_md_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_edit_linux_md_dialog_parent_class)->finalize (object);
}

static void
mdu_edit_linux_md_dialog_class_init (MduEditLinuxMdDialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduEditLinuxMdDialogPrivate));

        object_class->constructed  = mdu_edit_linux_md_dialog_constructed;
        object_class->finalize     = mdu_edit_linux_md_dialog_finalize;

        signals[ADD_SPARE_BUTTON_CLICKED_SIGNAL] =
                g_signal_new ("add-spare-button-clicked",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduEditLinuxMdDialogClass, add_spare_button_clicked),
                              NULL,
                              NULL,
        ...
        signals[EXPAND_BUTTON_CLICKED_SIGNAL] =
                g_signal_new ("expand-button-clicked",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduEditLinuxMdDialogClass, expand_button_clicked),
                              NULL,
                              NULL,
                              g_cclosure_marshal_VOID__VOID,
                              G_TYPE_NONE,
        ...
        signals[ATTACH_BUTTON_CLICKED_SIGNAL] =
                g_signal_new ("attach-button-clicked",
                              G_TYPE_FROM_CLASS (klass),
                              G_SIGNAL_RUN_LAST,
                              G_STRUCT_OFFSET (MduEditLinuxMdDialogClass, attach_button_clicked),
                              NULL,
                              NULL,
...
static void
mdu_edit_linux_md_dialog_init (MduEditLinuxMdDialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog,
                                                    MDU_TYPE_EDIT_LINUX_MD_DIALOG,
                                                    MduEditLinuxMdDialogPrivate);
}

GtkWidget *
mdu_edit_linux_md_dialog_new (GtkWindow        *parent,
                              MduLinuxMdDrive  *linux_md_drive)
{
        g_return_val_if_fail (MDU_IS_LINUX_MD_DRIVE (linux_md_drive), NULL);
        return GTK_WIDGET (g_object_new (MDU_TYPE_EDIT_LINUX_MD_DIALOG,
...
static void
on_add_spare_button_clicked (MduButtonElement *button_element,
                                 gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
        g_signal_emit (dialog, signals[ADD_SPARE_BUTTON_CLICKED_SIGNAL], 0);
}

static void
on_expand_button_clicked (MduButtonElement *button_element,
                          gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
...
static void
on_component_attach_button_clicked (MduButtonElement *button_element,
                                    gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
        MduDevice *slave;
...
static void
on_component_remove_button_clicked (MduButtonElement *button_element,
                                    gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
        MduDevice *slave;
               ...
               gpointer         user_data)
{
        ...
        gchar *name;
        gchar *drive_name;
        MduPresentable *volume_for_slave;
        MduPresentable *drive_for_slave;
        gchar *markup;
        gchar color[16];
        GtkStateType state;
        ...
        mdu_util_get_mix_color (GTK_WIDGET (dialog->priv->components_tree_view), state, color, sizeof (color));

        name = mdu_presentable_get_vpd_name (volume_for_slave);
        if (drive_for_slave != NULL) {
                drive_name = mdu_presentable_get_vpd_name (drive_for_slave);
        } else {
                drive_name = g_strdup ("");
        }
...
static void
on_linux_md_drive_changed (MduPresentable   *presentable,
                           gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
        ...
        if (!mdu_drive_is_active (MDU_DRIVE (mdu_dialog_get_presentable (MDU_DIALOG (dialog))))) {
                gtk_dialog_response (GTK_DIALOG (dialog), GTK_RESPONSE_CLOSE);
        }
}

static void
on_linux_md_drive_removed (MduPresentable   *presentable,
                           gpointer          user_data)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (user_data);
...
static void
mdu_edit_linux_md_dialog_constructed (GObject *object)
{
        MduEditLinuxMdDialog *dialog = MDU_EDIT_LINUX_MD_DIALOG (object);
        MduLinuxMdDrive *linux_md_drive;
        ...
        linux_md_drive = MDU_LINUX_MD_DRIVE (mdu_dialog_get_presentable (MDU_DIALOG (dialog)));

        icon = mdu_presentable_get_icon (MDU_PRESENTABLE (linux_md_drive));
        ...
        name = mdu_presentable_get_name (MDU_PRESENTABLE (linux_md_drive));
        vpd_name = mdu_presentable_get_vpd_name (MDU_PRESENTABLE (linux_md_drive));
        ...
        element = mdu_details_element_new (_("Position:"), NULL, NULL);
        g_ptr_array_add (elements, element);
        ...
        button_element = mdu_button_element_new (GTK_STOCK_NEW,
                                                 _("Add _Spare"),
        ...
        if (G_OBJECT_CLASS (mdu_edit_linux_md_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_edit_linux_md_dialog_parent_class)->constructed (object);
}

/* ---------------------------------------------------------------------------------------------------- */

static MduDevice *
get_selected_component (MduEditLinuxMdDialog *dialog)
{
        ...
        pool = mdu_presentable_get_pool (mdu_dialog_get_presentable (MDU_DIALOG (dialog)));
        slave_device = mdu_pool_get_by_object_path (pool, component_objpath);
...
static void
update_tree (MduEditLinuxMdDialog *dialog)
{
        MduLinuxMdDrive *linux_md_drive;
        GtkTreeStore *store;
        GList *slaves;
        ...
        linux_md_drive = MDU_LINUX_MD_DRIVE (mdu_dialog_get_presentable (MDU_DIALOG (dialog)));
        slaves = mdu_linux_md_drive_get_slaves (linux_md_drive);
        ...
        for (l = slaves; l != NULL; l = l->next) {
                MduDevice *sd = MDU_DEVICE (l->data);
                GdkPixbuf *pixbuf;
                GtkTreeIter iter;
                MduPool *pool;
                MduPresentable *volume_for_slave;
                ...
                if (mdu_device_should_ignore (sd))
                        continue;

                pool = mdu_device_get_pool (sd);
                volume_for_slave = mdu_pool_get_volume_by_device (pool, sd);
                g_object_unref (pool);

                if (volume_for_slave == NULL) {
                        g_warning ("Cannot find volume for device `%s'", mdu_device_get_object_path (sd));
                        continue;
                }

                if (mdu_device_is_partition (sd)) {
                        MduDevice *drive_device;
                        drive_device = mdu_pool_get_by_object_path (pool, mdu_device_partition_get_slave (sd));
                        if (drive_device != NULL) {
                                drive_for_slave = mdu_pool_get_drive_by_device (pool, drive_device);
                                g_object_unref (drive_device);
                        } else {
                                g_warning ("No slave with objpath %s", mdu_device_partition_get_slave (sd));
                        }
                } else {
                        drive_for_slave = mdu_pool_get_drive_by_device (pool, sd);
                 }

                if (mdu_drive_is_active (MDU_DRIVE (linux_md_drive))) {
                        position = mdu_device_linux_md_component_get_position (sd);
                        if (position >= 0) {
                ...
                pixbuf = mdu_util_get_pixbuf_for_presentable (volume_for_slave, GTK_ICON_SIZE_SMALL_TOOLBAR);
                object_path = mdu_device_get_object_path (sd);

                slave_state_str = mdu_linux_md_drive_get_slave_state_markup (linux_md_drive, sd);
...
static void
update_details (MduEditLinuxMdDialog *dialog)
{
        MduLinuxMdDrive *linux_md_drive;
        MduDevice *slave_device;
        MduDevice *slave_drive_device;
        ...
        if (slave_device == NULL) {
                mdu_details_element_set_text (dialog->priv->component_smart_element, "–");
                mdu_details_element_set_icon (dialog->priv->component_smart_element, NULL);
                mdu_details_element_set_text (dialog->priv->component_position_element, "–");
                mdu_details_element_set_text (dialog->priv->component_state_element, "–");
                mdu_details_element_set_text (dialog->priv->component_device_element, "–");
                goto out;
        }

        linux_md_drive = MDU_LINUX_MD_DRIVE (mdu_dialog_get_presentable (MDU_DIALOG (dialog)));

        s = mdu_linux_md_drive_get_slave_state_markup (linux_md_drive, slave_device);
        if (s == NULL)
                s = g_strdup ("–");
        mdu_details_element_set_text (dialog->priv->component_state_element, s);
        g_free (s);

        if (mdu_device_is_partition (slave_device)) {
                MduPool *pool;
                pool = mdu_device_get_pool (slave_device);
                slave_drive_device = mdu_pool_get_by_object_path (pool, mdu_device_partition_get_slave (slave_device));
                g_object_unref (pool);
        } else {
                slave_drive_device = g_object_ref (slave_device);
        }

        if (mdu_device_drive_ata_smart_get_is_available (slave_drive_device) &&
            mdu_device_drive_ata_smart_get_time_collected (slave_drive_device) > 0) {
                gboolean highlight;

                s = mdu_util_ata_smart_status_to_desc (mdu_device_drive_ata_smart_get_status (slave_drive_device),
                                                       &highlight,
                                                       NULL,
                                                       &icon);
                if (highlight) {
                ...
                mdu_details_element_set_text (dialog->priv->component_smart_element, s);
                mdu_details_element_set_icon (dialog->priv->component_smart_element, icon);

                g_free (s);
                g_object_unref (icon);
        } else {
                /* Translators: Used when SMART is not supported on the RAID component */
                mdu_details_element_set_text (dialog->priv->component_smart_element, _("Not Supported"));
        ...
        if (mdu_drive_is_active (MDU_DRIVE (linux_md_drive))) {
                position = mdu_device_linux_md_component_get_position (slave_device);
                if (position >= 0) {
        ...
        mdu_details_element_set_text (dialog->priv->component_position_element, s);
        g_free (s);

        slave_device_file = mdu_device_get_device_file_presentation (slave_device);
        if (slave_device_file == NULL || strlen (slave_device_file) == 0)
                slave_device_file = mdu_device_get_device_file (slave_device);
        mdu_details_element_set_text (dialog->priv->component_device_element, slave_device_file);
 ...
 out:
        mdu_button_element_set_visible (dialog->priv->component_attach_button, show_component_attach_button);
        mdu_button_element_set_visible (dialog->priv->component_remove_button, show_component_remove_button);
