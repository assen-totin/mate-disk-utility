...
static void do_relayout (MduDetailsTable *table);

G_DEFINE_TYPE (MduDetailsTable, mdu_details_table, GTK_TYPE_HBOX)

static void
mdu_details_table_finalize (GObject *object)
{
        MduDetailsTable *table = MDU_DETAILS_TABLE (object);
        ...
        if (G_OBJECT_CLASS (mdu_details_table_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_details_table_parent_class)->finalize (object);
}

static void
mdu_details_table_get_property (GObject    *object,
                                guint       property_id,
                                GValue     *value,
                                GParamSpec *pspec)
{
        ...
        case PROP_NUM_COLUMNS:
                g_value_set_uint (value, mdu_details_table_get_num_columns (table));
                break;

        case PROP_ELEMENTS:
                g_value_take_boxed (value, mdu_details_table_get_elements (table));
                break;
...
static void
mdu_details_table_set_property (GObject      *object,
                                guint         property_id,
                                const GValue *value,
                                GParamSpec   *pspec)
{
        ...
        switch (property_id) {
        case PROP_NUM_COLUMNS:
                mdu_details_table_set_num_columns (table, g_value_get_uint (value));
                break;

        case PROP_ELEMENTS:
                mdu_details_table_set_elements (table, g_value_get_boxed (value));
                break;
...
static void
mdu_details_table_init (MduDetailsTable *table)
{
        table->priv = G_TYPE_INSTANCE_GET_PRIVATE (table,
                                                   MDU_TYPE_DETAILS_TABLE,
                                                   MduDetailsTablePrivate);

}

static void
mdu_details_table_constructed (GObject *object)
{
        MduDetailsTable *table = MDU_DETAILS_TABLE (object);
        ...
        if (G_OBJECT_CLASS (mdu_details_table_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_details_table_parent_class)->constructed (object);
}

static void
mdu_details_table_class_init (MduDetailsTableClass *klass)
{
        GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduDetailsTablePrivate));

        gobject_class->get_property        = mdu_details_table_get_property;
        gobject_class->set_property        = mdu_details_table_set_property;
        gobject_class->constructed         = mdu_details_table_constructed;
        gobject_class->finalize            = mdu_details_table_finalize;
...
GtkWidget *
mdu_details_table_new (guint      num_columns,
                       GPtrArray *elements)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_DETAILS_TABLE,
...
guint
mdu_details_table_get_num_columns (MduDetailsTable *table)
{
        g_return_val_if_fail (MDU_IS_DETAILS_TABLE (table), 0);
        return table->priv->num_columns;
}

GPtrArray *
mdu_details_table_get_elements (MduDetailsTable *table)
{
        g_return_val_if_fail (MDU_IS_DETAILS_TABLE (table), NULL);
        return table->priv->elements != NULL ? g_ptr_array_ref (table->priv->elements) : NULL;
}

void
mdu_details_table_set_num_columns (MduDetailsTable *table,
                                   guint            num_columns)
{
        g_return_if_fail (MDU_IS_DETAILS_TABLE (table));
...
void
mdu_details_table_set_elements (MduDetailsTable *table,
                                GPtrArray       *elements)
{
        g_return_if_fail (MDU_IS_DETAILS_TABLE (table));
        if (table->priv->elements != NULL)
...
static void on_details_element_changed (MduDetailsElement *element,
                                        ElementData       *data);
...
static void
on_details_element_changed (MduDetailsElement *element,
                            ElementData       *data)
{
        gchar *s;
        GIcon *icon;
        ...
        mdu_util_get_mix_color (data->heading_label, GTK_STATE_NORMAL, color, sizeof (color));

        s = g_strdup_printf ("<span fgcolor='%s'>%s</span>",
                             color,
                             mdu_details_element_get_heading (element));
        gtk_label_set_markup (GTK_LABEL (data->heading_label), s);
        g_free (s);

        text = mdu_details_element_get_text (element);
        if (text != NULL) {
        ...
        time = mdu_details_element_get_time (element);
        if (time > 0) {
                GTimeVal time_val;
                time_val.tv_sec = time;
                time_val.tv_usec = 0;
                mdu_time_label_set_time (MDU_TIME_LABEL (data->time_label), &time_val);
        ...
        icon = mdu_details_element_get_icon (element);
        if (icon != NULL) {
        ...
        action_text = mdu_details_element_get_action_text (element);
        action_uri = mdu_details_element_get_action_uri (element);
        action_tooltip = mdu_details_element_get_action_tooltip (element);
        ...
        if (mdu_details_element_get_is_spinning (element)) {
                mdu_spinner_start (MDU_SPINNER (data->spinner));
                gtk_widget_set_no_show_all (data->spinner, FALSE);
                gtk_widget_show (data->spinner);
        } else {
                mdu_spinner_stop (MDU_SPINNER (data->spinner));
                gtk_widget_set_no_show_all (data->spinner, TRUE);
                gtk_widget_hide (data->spinner);
        }

        progress = mdu_details_element_get_progress (element);
        if (progress >= 0.0) {
        ...
        widget = mdu_details_element_get_widget (element);
        if (widget != NULL) {
                GtkWidget *child;
...
static void
do_relayout (MduDetailsTable *table)
{
        GList *children;
        GList *l;
        guint n;
        ...
        for (n = 0; n < table->priv->elements->len; n++) {
                MduDetailsElement *element = MDU_DETAILS_ELEMENT (table->priv->elements->pdata[n]);
                ...
                data->time_label = mdu_time_label_new (NULL);
