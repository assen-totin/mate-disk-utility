...
static void do_relayout (MduButtonTable *table);

G_DEFINE_TYPE (MduButtonTable, mdu_button_table, GTK_TYPE_HBOX)

static void
mdu_button_table_finalize (GObject *object)
{
        MduButtonTable *table = MDU_BUTTON_TABLE (object);
        ...
        if (G_OBJECT_CLASS (mdu_button_table_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_button_table_parent_class)->finalize (object);
}

static void
mdu_button_table_get_property (GObject    *object,
                               guint       property_id,
                               GValue     *value,
                               GParamSpec *pspec)
{
        ...
        case PROP_NUM_COLUMNS:
                g_value_set_uint (value, mdu_button_table_get_num_columns (table));
                break;

        case PROP_ELEMENTS:
                g_value_take_boxed (value, mdu_button_table_get_elements (table));
                break;
...
static void
mdu_button_table_set_property (GObject      *object,
                               guint         property_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
        ...
        switch (property_id) {
        case PROP_NUM_COLUMNS:
                mdu_button_table_set_num_columns (table, g_value_get_uint (value));
                break;

        case PROP_ELEMENTS:
                mdu_button_table_set_elements (table, g_value_get_boxed (value));
                break;
...
static void
mdu_button_table_init (MduButtonTable *table)
{
        table->priv = G_TYPE_INSTANCE_GET_PRIVATE (table,
                                                   MDU_TYPE_BUTTON_TABLE,
                                                   MduButtonTablePrivate);

}

static void
mdu_button_table_constructed (GObject *object)
{
        MduButtonTable *table = MDU_BUTTON_TABLE (object);
        ...
        if (G_OBJECT_CLASS (mdu_button_table_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_button_table_parent_class)->constructed (object);
}

static void
mdu_button_table_class_init (MduButtonTableClass *klass)
{
        GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduButtonTablePrivate));

        gobject_class->get_property        = mdu_button_table_get_property;
        gobject_class->set_property        = mdu_button_table_set_property;
        gobject_class->constructed         = mdu_button_table_constructed;
        gobject_class->finalize            = mdu_button_table_finalize;
...
GtkWidget *
mdu_button_table_new (guint      num_columns,
                      GPtrArray *elements)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_BUTTON_TABLE,
...
guint
mdu_button_table_get_num_columns (MduButtonTable *table)
{
        g_return_val_if_fail (MDU_IS_BUTTON_TABLE (table), 0);
        return table->priv->num_columns;
}

GPtrArray *
mdu_button_table_get_elements (MduButtonTable *table)
{
        g_return_val_if_fail (MDU_IS_BUTTON_TABLE (table), NULL);
        return table->priv->elements != NULL ? g_ptr_array_ref (table->priv->elements) : NULL;
}

void
mdu_button_table_set_num_columns (MduButtonTable *table,
                                   guint            num_columns)
{
        g_return_if_fail (MDU_IS_BUTTON_TABLE (table));
...
void
mdu_button_table_set_elements (MduButtonTable *table,
                                GPtrArray       *elements)
{
        g_return_if_fail (MDU_IS_BUTTON_TABLE (table));
        if (table->priv->elements != NULL)
...
typedef struct {
        MduButtonTable *table;
        MduButtonElement *element;

        GtkWidget *button;
} ElementData;

static void on_button_element_changed (MduButtonElement *element,
                                       ElementData       *data);
...
static void
on_button_element_changed (MduButtonElement *element,
                           ElementData       *data)
{
               ...
               const gchar *button_secondary)
{
        ...
        mdu_util_get_mix_color (widget, GTK_STATE_NORMAL, color, sizeof (color));
...
static void
do_relayout (MduButtonTable *table)
{
        GList *children;
        GList *l;
        guint n;
        ...
        for (n = 0, m = 0; n < table->priv->elements->len; n++) {
                MduButtonElement *element = MDU_BUTTON_ELEMENT (table->priv->elements->pdata[n]);
                ...
                if (!mdu_button_element_get_visible (element)) {
                        continue;
                }
                ...
                data->button = create_button (GTK_WIDGET (table),
                                              mdu_button_element_get_icon_name (element),
                                              mdu_button_element_get_primary_text (element),
                                              mdu_button_element_get_secondary_text (element));
