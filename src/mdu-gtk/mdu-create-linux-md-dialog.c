...
#include "mdu-create-linux-md-dialog.h"
#include "mdu-size-widget.h"
#include "mdu-disk-selection-widget.h"

struct MduCreateLinuxMdDialogPrivate
{
        MduPool *pool;
...
static void mdu_create_linux_md_dialog_constructed (GObject *object);

static void update (MduCreateLinuxMdDialog *dialog);

static void get_sizes (MduCreateLinuxMdDialog *dialog,
                       guint                  *out_num_disks,
...
G_DEFINE_TYPE (MduCreateLinuxMdDialog, mdu_create_linux_md_dialog, GTK_TYPE_DIALOG)

static void
mdu_create_linux_md_dialog_finalize (GObject *object)
{
        MduCreateLinuxMdDialog *dialog = MDU_CREATE_LINUX_MD_DIALOG (object);

        g_object_unref (dialog->priv->pool);
        g_free (dialog->priv->level);

        if (G_OBJECT_CLASS (mdu_create_linux_md_dialog_parent_class)->finalize != NULL)
                G_OBJECT_CLASS (mdu_create_linux_md_dialog_parent_class)->finalize (object);
}

static void
mdu_create_linux_md_dialog_get_property (GObject    *object,
                                         guint       property_id,
                                         GValue     *value,
                                         GParamSpec *pspec)
{
        ...
        case PROP_NAME:
                g_value_set_string (value, mdu_create_linux_md_dialog_get_name (dialog));
                break;

        case PROP_SIZE:
                g_value_set_uint64 (value, mdu_create_linux_md_dialog_get_size (dialog));
                break;

        case PROP_COMPONENT_SIZE:
                g_value_set_uint64 (value, mdu_create_linux_md_dialog_get_component_size (dialog));
                break;

        case PROP_STRIPE_SIZE:
                g_value_set_uint64 (value, mdu_create_linux_md_dialog_get_stripe_size (dialog));
                break;

        case PROP_DRIVES:
                g_value_set_boxed (value, mdu_create_linux_md_dialog_get_drives (dialog));
                break;
...
static void
mdu_create_linux_md_dialog_set_property (GObject      *object,
                                         guint         property_id,
                                         const GValue *value,
                                         GParamSpec   *pspec)
{
...
static void
mdu_create_linux_md_dialog_class_init (MduCreateLinuxMdDialogClass *klass)
{
        GObjectClass *object_class = G_OBJECT_CLASS (klass);

        g_type_class_add_private (klass, sizeof (MduCreateLinuxMdDialogPrivate));

        object_class->get_property = mdu_create_linux_md_dialog_get_property;
        object_class->set_property = mdu_create_linux_md_dialog_set_property;
        object_class->constructed  = mdu_create_linux_md_dialog_constructed;
        object_class->finalize     = mdu_create_linux_md_dialog_finalize;
...
static void
mdu_create_linux_md_dialog_init (MduCreateLinuxMdDialog *dialog)
{
        dialog->priv = G_TYPE_INSTANCE_GET_PRIVATE (dialog, MDU_TYPE_CREATE_LINUX_MD_DIALOG, MduCreateLinuxMdDialogPrivate);
}

GtkWidget *
mdu_create_linux_md_dialog_new (GtkWindow *parent,
                                MduPool   *pool)
{
        return GTK_WIDGET (g_object_new (MDU_TYPE_CREATE_LINUX_MD_DIALOG,
...
gchar *
mdu_create_linux_md_dialog_get_level  (MduCreateLinuxMdDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_CREATE_LINUX_MD_DIALOG (dialog), NULL);
        return g_strdup (dialog->priv->level);
}

gchar *
mdu_create_linux_md_dialog_get_name (MduCreateLinuxMdDialog *dialog)
{
        g_return_val_if_fail (MDU_IS_CREATE_LINUX_MD_DIALOG (dialog), NULL);
...
guint64
mdu_create_linux_md_dialog_get_size (MduCreateLinuxMdDialog  *dialog)
{
        guint64 array_size;

        g_return_val_if_fail (MDU_IS_CREATE_LINUX_MD_DIALOG (dialog), 0);
...
guint64
mdu_create_linux_md_dialog_get_component_size (MduCreateLinuxMdDialog  *dialog)
{
...
guint64
mdu_create_linux_md_dialog_get_stripe_size (MduCreateLinuxMdDialog  *dialog)
{
        g_return_val_if_fail (MDU_IS_CREATE_LINUX_MD_DIALOG (dialog), 0);
...
GPtrArray *
mdu_create_linux_md_dialog_get_drives (MduCreateLinuxMdDialog  *dialog)
{
        g_return_val_if_fail (MDU_IS_CREATE_LINUX_MD_DIALOG (dialog), NULL);

        return mdu_disk_selection_widget_get_selected_drives (MDU_DISK_SELECTION_WIDGET (dialog->priv->disk_selection_widget));
}
...
on_level_combo_box_changed (GtkWidget *combo_box,
                            gpointer   user_data)
{
        MduCreateLinuxMdDialog *dialog = MDU_CREATE_LINUX_MD_DIALOG (user_data);
        gchar *s;
        gchar *markup;
        ...
        s = mdu_linux_md_get_raid_level_description (dialog->priv->level);
...
static void
mdu_create_linux_md_dialog_constructed (GObject *object)
{
        MduCreateLinuxMdDialog *dialog = MDU_CREATE_LINUX_MD_DIALOG (object);
        ...
        gtk_window_set_icon_name (GTK_WINDOW (dialog), "mdu-raid-array");
        ...
        s = mdu_linux_md_get_raid_level_for_display ("raid0", TRUE);
        gtk_combo_box_append_text (GTK_COMBO_BOX (combo_box), s);
        g_free (s);
        s = mdu_linux_md_get_raid_level_for_display ("raid1", TRUE);
        gtk_combo_box_append_text (GTK_COMBO_BOX (combo_box), s);
        g_free (s);
        s = mdu_linux_md_get_raid_level_for_display ("raid5", TRUE);
        gtk_combo_box_append_text (GTK_COMBO_BOX (combo_box), s);
        g_free (s);
        s = mdu_linux_md_get_raid_level_for_display ("raid6", TRUE);
        gtk_combo_box_append_text (GTK_COMBO_BOX (combo_box), s);
        g_free (s);
        ...
        disk_selection_widget = mdu_disk_selection_widget_new (dialog->priv->pool,
                                                               MDU_DISK_SELECTION_WIDGET_FLAGS_ALLOW_MULTIPLE |
        ...
        if (G_OBJECT_CLASS (mdu_create_linux_md_dialog_parent_class)->constructed != NULL)
                G_OBJECT_CLASS (mdu_create_linux_md_dialog_parent_class)->constructed (object);
}

/* ---------------------------------------------------------------------------------------------------- */

static void
get_sizes (MduCreateLinuxMdDialog *dialog,
           guint                  *out_num_disks,
           guint                  *out_num_available_disks,
           guint64                *out_component_size,
           guint64                *out_array_size,
           ...
           guint64                *out_max_array_size)
{
        ...
        selected_disks = mdu_disk_selection_widget_get_selected_drives (MDU_DISK_SELECTION_WIDGET (dialog->priv->disk_selection_widget));

        num_disks = selected_disks->len;
        num_available_disks = mdu_disk_selection_widget_get_num_available_disks (MDU_DISK_SELECTION_WIDGET (dialog->priv->disk_selection_widget));
        max_component_size = mdu_disk_selection_widget_get_largest_segment_for_selected (MDU_DISK_SELECTION_WIDGET (dialog->priv->disk_selection_widget));
        ...
        array_size = mdu_size_widget_get_size (MDU_SIZE_WIDGET (dialog->priv->size_widget));
        if (max_array_size > 0)
...
static void
update (MduCreateLinuxMdDialog *dialog)
{
        gchar *tip_text;
        ...
        level_str = mdu_linux_md_get_raid_level_for_display (dialog->priv->level, FALSE);

        old_size = mdu_size_widget_get_size (MDU_SIZE_WIDGET (dialog->priv->size_widget));
        old_max_size = mdu_size_widget_get_max_size (MDU_SIZE_WIDGET (dialog->priv->size_widget));
        ...
        if (num_available_disks < dialog->priv->num_disks_needed) {
                ...
                mdu_size_widget_set_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), 0);
                mdu_size_widget_set_min_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), 0);
                mdu_size_widget_set_max_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), 100);
                ...
                mdu_size_widget_set_min_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), 0);
                mdu_size_widget_set_max_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), max_array_size);

                if (tip_text == NULL) {
                        gchar *strsize;

                        strsize = mdu_util_get_size_for_display (array_size, FALSE, FALSE);
                        /* Translators: This is for the tip text shown in the dialog.
        ...
        if (max_array_size > 0 &&
            (max_array_size > old_max_size && (was_at_zero || was_at_max)))
                mdu_size_widget_set_size (MDU_SIZE_WIDGET (dialog->priv->size_widget), max_array_size);
